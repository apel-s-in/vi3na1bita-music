...
// UID.002_(UID-first core)_(оставить избранное строго uid-based)_(никаких filename/title/index ключей, только uid)
// UID.017_(Launch source stats)_(подготовить future аналитику источников лайка/анлайка)_(в будущем действия favorites могут логироваться как source-aware events)
// UID.051_(Collection state)_(связать избранное с collectible-слоем)_(favorite badge и completion будут выводиться поверх этого truth-слоя)
// UID.052_(Track badges and completion)_(дать stable source для favorite-badge)_(listener collection/intel не должны дублировать favorite truth)
// UID.062_(Recommendation memory and feedback)_(использовать избранное как сильный preference signal)_(listener/recs слой должен читать snapshot отсюда, не хранить дубликат truth)
// UID.094_(No-paralysis rule)_(оставить избранное независимым от intel-слоя)_(если intel/recs/providers отключены, favorites работают как раньше)
