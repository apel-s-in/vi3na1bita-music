...
export const YandexAuth = {
  ...
  login() {
    ...
    // Открываем popup — CSP родительской страницы не мешает Яндексу
    const w = 520, h = 620;
    ...
    if (!popup) {
      // Fallback если popup заблокирован браузером
      window.NotificationSystem?.info('Разрешите всплывающие окна для входа через Яндекс, или перейдите по ссылке вручную.');
      window.open(url, '_blank');
      return;
    }

    // Следим за URL popup — когда Яндекс редиректит обратно с токеном
    const timer = setInterval(async () => {
      try {
        // Как только popup перешёл на наш домен — читаем hash
        if (popup.closed) { clearInterval(timer); return; }
        const popupUrl = popup.location.href;
        if (popupUrl.startsWith(REDIRECT_URI) && popupUrl.includes('access_token')) {
          clearInterval(timer);
        ...
        // Cross-origin — popup ещё на Яндексе, игнорируем
      }
    }, 500);

    // Таймаут 5 минут — закрываем слежку
    setTimeout(() => { clearInterval(timer); if (!popup.closed) popup.close(); }, 300000);
  },
