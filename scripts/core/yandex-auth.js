...
const avatarUrlFromProfile=p=>{const id=String(p?.default_avatar_id||p?.avatar_id||'').trim();if(!id||p?.is_avatar_empty===true||id==='0/0-0')return null;return `https://avatars.yandex.net/get-yapic/${id.replace(/^\/+/,'')}/islands-200`;};
const spin='<span class="ya-preload-spinner" style="display:inline-block;width:12px;height:12px;border:2px solid rgba(77,170,255,.2);border-top-color:#4daaff;border-radius:50%;animation:spin .8s linear infinite"></span>';
export const YandexAuth={
  ...
  async _onFirstLogin(yP,t){
    const yId=String(yP.id||'').trim(),rN=String(yP.real_name||yP.display_name||yP.login||'').trim(),lg=String(yP.login||'').trim(),av=avatarUrlFromProfile(yP);
  ...
  _showNamePickModal(rN,lg){
    if(!window.Modals?.open)return;const sg=rN||lg||'Слушатель',esc=s=>window.Utils?.escapeHtml?.(String(s||''))||String(s||''),m=window.Modals.open({title:'👋 Добро пожаловать!',maxWidth:400,strictClose:true,bodyHtml:`<div style="color:#9db7dd;margin-bottom:16px;line-height:1.5">Вы вошли через Яндекс.<br>Как вас отображать в приложении?</div><div style="margin-bottom:14px"><label style="font-size:12px;color:#888;display:block;margin-bottom:6px">Ваше имя</label><input type="text" id="ya-display-name" style="width:100%;padding:10px 14px;border-radius:10px;background:rgba(255,255,255,.08);border:1px solid rgba(255,255,255,.15);color:#fff;font-size:16px;outline:none" maxlength="20" placeholder="${esc(sg)}" value="${esc(sg)}" autocomplete="off"><div style="font-size:11px;color:#666;margin-top:6px">Или оставьте как есть — ${esc(sg)}</div></div><div class="modal-choice-actions profile-inline-actions"><button class="modal-action-btn online" id="ya-name-save" disabled>Сохранить</button></div><div id="ya-preload-status" style="margin-top:10px;font-size:11px;color:#666;text-align:center;min-height:16px;display:flex;align-items:center;justify-content:center;gap:6px">${spin} Проверяем облачную копию...</div>`}),i=m.querySelector('#ya-display-name'),b=m.querySelector('#ya-name-save');
    const setSave=on=>{if(b){b.disabled=!on;b.style.opacity=on?'':'0.45';b.style.cursor=on?'pointer':'not-allowed';}};
    setTimeout(()=>i?.focus(),100);setSave(false);try{sessionStorage.removeItem(NEW_DEVICE_CONFIRMED_KEY)}catch{}
    const updatePreloadStatus=async(attemptRetry=false)=>{
      try{
        const mod=await import('../app/profile/auth-onboarding-orchestrator.js'),statusEl=m.querySelector('#ya-preload-status');
        if(!statusEl)return;
        setSave(false);
        if(attemptRetry){statusEl.innerHTML=`${spin} Повторяем проверку...`;mod.clearPreloadCache?.();mod.startPreload?.(this.getToken());}
        const result=await mod._waitForPreload?.();if(!m.isConnected)return;
        if(result?.meta){statusEl.innerHTML='<span style="color:#4caf50">✓</span> Облачная копия найдена';setSave(true);return;}
        statusEl.innerHTML='<div style="display:flex;flex-direction:column;gap:8px;align-items:stretch;width:100%"><div><span style="color:#888">○</span> Облачной копии нет</div><div style="display:flex;gap:8px"><button type="button" class="om-btn om-btn--ghost" id="ya-preload-retry" style="flex:1;font-size:11px;padding:7px 8px">Повторить</button><button type="button" class="om-btn om-btn--primary" id="ya-new-device-ok" style="flex:1;font-size:11px;padding:7px 8px">Новое устройство</button></div></div>';
        m.querySelector('#ya-preload-retry')?.addEventListener('click',e=>{e.preventDefault();updatePreloadStatus(true);});
        m.querySelector('#ya-new-device-ok')?.addEventListener('click',e=>{e.preventDefault();try{sessionStorage.setItem(NEW_DEVICE_CONFIRMED_KEY,'1')}catch{}statusEl.innerHTML='<span style="color:#4caf50">✓</span> Новое устройство выбрано';setSave(true);});
      }catch{}
    };
    updatePreloadStatus(false);
    const sv=async()=>{
      if(b?.disabled)return window.NotificationSystem?.warning?.('Дождитесь проверки backup или выберите «Новое устройство»');
      ...
      setTimeout(async()=>{try{const {runOnboardingFlow}=await import('../app/profile/auth-onboarding-orchestrator.js');await runOnboardingFlow({token:this.getToken(),profile:read(LS_PROFILE),isFirstLogin:true});}catch(e){console.warn('[YandexAuth] onboarding flow failed:',e?.message);}},200);
    };
    b?.addEventListener('click',sv);i?.addEventListener('keydown',e=>{if(e.key==='Enter')sv();});
  },
