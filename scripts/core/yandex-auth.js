...
export const YandexAuth={
  ...
  async _onFirstLogin(yP,t){
    ...
    try{
      const {YandexDisk:YD}=await import('./yandex-disk.js');
      const m=await YD.getMeta(t).catch(()=>null);
      if(m){
        ...
        window.dispatchEvent(new CustomEvent('yandex:cloud:newer',{detail:{meta:m,compareState:'cloud_richer_new_device'}}));
      }
    }catch(e){console.debug('[AutoRestore meta-only] skip:',e)}
