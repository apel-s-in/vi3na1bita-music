...
export const YandexAuth={
  ...
  async _onFirstLogin(yP,t){
    ...
    try{
      ...
      const{YandexDisk:YD}=await import('./yandex-disk.js'),m=await YD.getMeta(t).catch(()=>null);
      if(m){
        window.NotificationSystem?.info('Скачивание профиля...');const d=await YD.download(t).catch(()=>null);
        if(d){
          const{BackupVault:BV}=await import('../analytics/backup-vault.js');await BV.importData(new Blob([JSON.stringify(d)]),'all');
          localStorage.setItem('yandex:last_backup_check',JSON.stringify(m));localStorage.setItem('yandex:last_backup_meta',JSON.stringify(m));localStorage.setItem('yandex:last_backup_local_ts',String(Number(d.revision?.timestamp||d.createdAt||Date.now())));
          const{markSyncReady:mSR,markRestoreOrSkipDone:mROSD}=await import('../analytics/backup-sync-engine.js');mSR('auto_restore');mROSD('auto_restore');
          window.NotificationSystem?.success('Прогресс автоматически восстановлен! Перезагрузка...');setTimeout(()=>window.location.reload(),1500);return;
        }
      }
    }catch(e){console.debug('[AutoRestore] skip:',e)}
    window.dispatchEvent(new CustomEvent('yandex:auth:changed',{detail:{status:'active',profile:read(LS_PROFILE)}}));this._showNamePickModal(rN,lg);
  },
  _showNamePickModal(rN,lg){
    ...
    const sv=()=>{const n=i?.value?.trim()||sg,p=read(LS_PROFILE)||{};write(LS_PROFILE,{...p,displayName:n});window.dispatchEvent(new CustomEvent('yandex:auth:changed',{detail:{status:'active',profile:read(LS_PROFILE)}}));window.NotificationSystem?.success(`Имя сохранено: ${n} ✅`);m.remove()};
