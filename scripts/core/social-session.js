...
const currentDeviceContext = () => {
  ...
  return {
    deviceId: safe(localStorage.getItem('deviceStableId') || localStorage.getItem('deviceHash') || 'web'),
    deviceLabel: safe(localStorage.getItem('yandex:onboarding:device_label') || (info.isIOS ? 'Мой iPhone' : info.isAndroid ? 'Моё Android устройство' : 'Мой Desktop')),
    deviceClass: info.isIOS ? 'iPhone' : info.isAndroid ? 'Android' : 'Desktop',
    platform: info.isIOS ? 'ios' : info.isAndroid ? 'android' : 'web',
    pwa: info.isPWA === true,
...
export const getSocialSession = async ({ force = false } = {}) => {
  ...
  pendingSession = (async () => {
    const response = await fetch(SIGNALING_URL, {
      ...
      credentials: 'omit',
      mode: 'cors',
      body: JSON.stringify({ action: 'social_session_issue', displayName: profile.displayName, avatarUrl: profile.avatar, ...currentDeviceContext() })
    });
