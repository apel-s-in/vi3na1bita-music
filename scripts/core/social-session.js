...
const coalescedActions = new Set(['achievement_reward_status', 'favorite_state_get', 'wallet_get']);
...
export const getSocialSession = async ({ force = false } = {}) => {
  ...
  pendingSession = (async () => {
    const response = await fetch(SIGNALING_URL, {
      ...
      credentials: 'omit',
      mode: 'cors',
      body: JSON.stringify({ action: 'social_session_issue', displayName: profile.displayName, avatarUrl: profile.avatar, deviceId: localStorage.getItem('deviceStableId') || localStorage.getItem('deviceHash') || 'web' })
    });
