...
const API='https://cloud-api.yandex.net/v1/disk',BD=CLOUD_BACKUP_DIR,BP=CLOUD_BACKUP_LATEST_PATH,MP=CLOUD_BACKUP_META_PATH,BPV=s=>buildBackupHistoryPath(s),PROXY='https://functions.yandexcloud.net/d4ecdu6kgamevcauajid',aH=t=>({'Authorization':`OAuth ${t}`}),sS=safeCloudString,sN=safeCloudNum,dS=safeDeviceString,sPJ=t=>{try{return JSON.parse(t)}catch{return null}},mPE=(p,e)=>{const s=Number(e?.status||0),pl=e?.payload&&typeof e.payload==='object'?e.payload:null,d=[sS(pl?.error),sS(pl?.stage),sS(pl?.path),sS(pl?.raw),sS(pl?.hint)].filter(Boolean).join(' | '),err=new Error(d?`${p}:${d}`:p);err.status=s;err.payload=pl;return err};
...
export const YandexDisk={
  ...
  async getMeta(t){if(!t)throw new Error('no_token');const u=new URL(PROXY);u.searchParams.set('mode','meta');try{const d=await fPJ(u.toString(),t,2);return d?.exists?normalizeCloudBackupMeta(d.latest||{}):null}catch(e){const s=Number(e?.status||0);if(s===404)return null;if(s===401)throw mPE('disk_auth_error',e);if(s===403)throw mPE('disk_forbidden',e);throw mPE(sS(e?.message||'meta_proxy_failed'),e)}},
  async uploadDeviceSettings(t,d){if(!t)throw new Error('no_token');const stableId=dS(d?.deviceStableId||'');const path=buildDeviceSettingsPath(stableId);if(!path)throw new Error('device_settings_no_stable_id');const doc=normalizeDeviceSettingsSnapshot({...d,path});await pJP(t,path,doc);return doc},
  async getDeviceSettingsMeta(t,stableId){if(!t)throw new Error('no_token');const sid=dS(stableId);if(!sid)throw new Error('device_settings_no_stable_id');const u=new URL(PROXY);u.searchParams.set('mode','device_meta');u.searchParams.set('deviceStableId',sid);try{const d=await fPJ(u.toString(),t,2);return d?.exists?normalizeDeviceSettingsSnapshot(d.device||{}):null}catch(e){const s=Number(e?.status||0);if(s===404)return null;if(s===401)throw mPE('disk_auth_error',e);if(s===403)throw mPE('disk_forbidden',e);throw mPE(sS(e?.message||'device_meta_proxy_failed'),e)}},
  async downloadDeviceSettings(t,stableId){if(!t)throw new Error('no_token');const sid=dS(stableId);if(!sid)throw new Error('device_settings_no_stable_id');const u=new URL(PROXY);u.searchParams.set('mode','device_download');u.searchParams.set('deviceStableId',sid);try{const d=await fPJ(u.toString(),t,2);if(!d||typeof d!=='object')throw new Error('invalid_device_settings_payload');return normalizeDeviceSettingsSnapshot(d)}catch(e){const s=Number(e?.status||0);if(s===404)throw mPE('device_settings_not_found',e);if(s===401)throw mPE('disk_auth_error',e);if(s===403)throw mPE('disk_forbidden',e);if([502,503,504].includes(s))throw mPE('device_download_proxy_failed',e);throw mPE(sS(e?.message||'device_download_proxy_failed'),e)}},
  async download(t,p=BP){if(!t)throw new Error('no_token');const u=new URL(PROXY);u.searchParams.set('mode','download');u.searchParams.set('path',sanitizeBackupPath(p));try{const d=await fPJ(u.toString(),t,2);if(!d||typeof d!=='object')throw new Error('invalid_backup_payload');return d}catch(e){const s=Number(e?.status||0);if(s===404)throw mPE('backup_not_found',e);if(s===401)throw mPE('disk_auth_error',e);if(s===403)throw mPE('disk_forbidden',e);if([502,503,504].includes(s))throw mPE('proxy_failed_or_timeout',e);throw mPE(sS(e?.message||'proxy_failed_or_timeout'),e)}},
  async listBackups(t){if(!t)throw new Error('no_token');const u=new URL(PROXY);u.searchParams.set('mode','list');try{const d=await fPJ(u.toString(),t,2);return Array.isArray(d?.items)?d.items.map(normalizeCloudBackupListItem):[]}catch(e){const s=Number(e?.status||0);if(s===401)throw mPE('disk_auth_error',e);if(s===403)throw mPE('disk_forbidden',e);if([502,503,504].includes(s))throw mPE('list_proxy_failed',e);throw mPE(sS(e?.message||'list_proxy_failed'),e)}},
