...
let booted = false;
export async function initIntelBootstrap({ W = window, C = W.APP_CONFIG || {} } = {}) {
  if (booted && W.Intel) return W.Intel;
  const flags = getIntelFlags(C);
  const api = {
    version: 'intel-layer-v1', roadmapVersion: INTEL_ROADMAP_VERSION, flags, contracts: INTEL_CONTRACTS, bus: intelBus, guards: intelGuards, helpers: { makeNoopModule, makeMemoryState, makeJsonStorage, safeEmit }, trackProfiles, trackPresentation, trackRelations, listenerProfile, listenerCollection, recommendationEngine, recommendationStrategies, providerConsents, providerIdentity, hybridSync, providerActions, telemetryMapper, cohorts, similarListeners, communityStats, ui: { trackProfileModal, profileInsights, showcaseSemantic },
    disable() { setIntelDisabled(true, C); intelBus.emit('disabled', { reason: 'manual' }); return true; },
    enable() { setIntelDisabled(false, C); intelBus.emit('ready', { reason: 'manual-enable-request' }); return true; }
  };
  W.Intel = W.IntelLayer = api;
  if (!flags.enabled) { booted = true; intelBus.emit('disabled', { reason: flags.reason }); return api; }
  await Promise.allSettled([ trackProfiles.init(api), trackPresentation.init(api), trackRelations.init(api), listenerProfile.init(api), listenerCollection.init(api), recommendationEngine.init(api), providerConsents.init(api), providerIdentity.init(api), hybridSync.init(api), providerActions.init(api), telemetryMapper.init(api), cohorts.init(api), similarListeners.init(api), communityStats.init(api), trackProfileModal.init(api), profileInsights.init(api), showcaseSemantic.init(api) ]);
  booted = true; intelBus.emit('ready', { version: api.version, roadmapVersion: INTEL_ROADMAP_VERSION });
  return api;
}
