// UID.004_(Stats as cache)_(statistics modal должен читать stats-state view model)_(не дублировать расчёты byHour/top/totals)
...
export async function openStatisticsModal(uid = null) {
  ...
  const vm = buildStatsViewModel(await metaDB.getAllStats()), engine = window.achievementEngine, f = vm.globalFeatures;
  window.Modals.open({
