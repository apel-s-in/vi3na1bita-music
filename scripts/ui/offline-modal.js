...
function _buildModesSection(stats, mgr) {
  ...
  section.innerHTML = `
    ...
    <div class="offline-row">
      <span class="offline-row__label">Текущий режим</span>
      <span style="font-weight: 600;">${mode}</span>
    </div>
    <div style="margin-top: 8px; font-size: 12px; color: #888;">
      R0 = Стриминг &nbsp;|&nbsp; R1 = PlaybackCache &nbsp;|&nbsp; R2 = Dynamic Offline &nbsp;|&nbsp; R3 = 100% Offline
    </div>
  `;

  /* Mode buttons */
  const modeRow = document.createElement('div');
  modeRow.style.cssText = 'display: flex; gap: 4px; margin-top: 10px; flex-wrap: wrap;';

  for (const m of ['R0', 'R1', 'R2', 'R3']) {
    const btn = document.createElement('button');
    btn.className = 'offline-btn' + (m === mode ? ' offline-btn--active' : '');
    btn.textContent = m;
    btn.addEventListener('click', async () => {
      if (m === 'R3') {
        if (!confirm('Переключиться на 100% Offline? Потребуется загрузка всех треков.')) return;
      }
      await mgr.setMode(m);
      /* Перерендерить модал */
      closeOfflineModal();
