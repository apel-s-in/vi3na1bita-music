...
async function handleAction(e) {
  ...
  const handleQ = async (nq, isR2) => {
    ...
    let count = 0, bytes = 0;
    (await getAllTrackMetas()).forEach(m => { if (['pinned', 'cloud', 'dynamic'].includes(m.type) && (String(m.quality || '').toLowerCase() === 'lo' ? 'lo' : 'hi') !== nq) { count++; bytes += (m.size || 0); } });
    const apply = () => { isR2 ? om.setCQ(nq) : window.playerCore?.switchQuality?.(nq); refresh(); };
    count > 5 ? confirmBox({ title: 'Смена качества', textHtml: `Смена затронет ${count} файлов (${fMB(bytes)}). Перекачать?`, confirmText: 'Перекачать', onConfirm: apply }) : apply();
  };
  ...
  const actions = {
    ...
    'recache': async () => {
      const isR2 = om.getMode() === 'R2', targetQ = isR2 ? om.getCQ() : om.getQuality(), metas = await getAllTrackMetas(), winUids = Object.values(window.PlaybackCache?.getWindowState?.() || {}).filter(Boolean);
      let uidCount = 0, bytes = 0;
      for (const m of metas) if (m?.cachedComplete && ['pinned', 'cloud', 'dynamic', 'playbackCache'].includes(m.type) && (String(m.quality || '').toLowerCase() === 'lo' ? 'lo' : 'hi') !== targetQ) { uidCount++; bytes += (m.size || 0); }
      const doRun = async () => { if (om.queue) om.queue.setParallel(3); await om.reCacheAll(targetQ); setTimeout(() => om.queue?.setParallel?.(1), 15000); refresh(); };
      if (!isR2) return doRun();
      confirmBox({ title: 'Re-cache (R2)', textHtml: `Будут перекачаны треки в качестве ${targetQ === 'lo' ? 'Lo' : 'Hi'}.<br>Порядок: окно PREV/CUR/NEXT → 🔒 pinned → ☁ cloud → 🧠 dynamic.<br>${winUids.length ? `Окно: ${winUids.length} UID.` : ''}`, confirmText: 'Далее', cancelText: 'Отмена', onConfirm: () => confirmBox({ title: 'Re-cache (R2)', textHtml: `Будет перекачано: <strong>${uidCount}</strong> UID<br>Объём: <strong>${fMB(bytes)}</strong>`, confirmText: 'Начать', cancelText: 'Отмена', onConfirm: doRun }) });
    },
