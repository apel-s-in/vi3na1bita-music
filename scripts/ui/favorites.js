...
(function FavoritesUIModule() {
  ...
  function updateFavoritesCurrentRow(idx) {
    ...
    try {
      ...
      const origIdx = (Array.isArray(window.favPlayableMap) && Number.isInteger(window.favPlayableMap[idx])) 
        ? window.favPlayableMap[idx] 
        : idx;
    ...
    // Размещение блока плеера: в мини‑режиме держим ВВЕРХУ (#now-playing), иначе — под текущей строкой
    try {
      const lp = document.getElementById('lyricsplayerblock');
      if (!lp) {
        // Если блок плеера не существует - создаем его
        renderLyricsBlock();
        return;
      }
      ...
      if (rowUnder && lp && rowUnder.parentNode) {
        // Проверяем, находится ли блок плеера в правильном месте
  ...
  async function ensureFavoritesPlayback(target) {
    ...
    if (!Array.isArray(window.favoritesRefsModel) || window.favoritesRefsModel.length === 0) {
      await (window.FavoritesData?.buildFavoritesRefsModel?.() ?? Promise.resolve([]));
    }
    ...
    if (!playable.length) {
      window.NotificationSystem && window.NotificationSystem.warning('Нет активных треков со ⭐');
      return;
    }
    ...
    if (typeof window.updateAvailableTracks === 'function') window.updateAvailableTracks();
    ...
    if (window.__useNewPlayerCore && window.playerCore) {
      try {
        // Подготовка плейлиста
        const payload = __buildPlayerCorePayload();
        if (payload) {
          window.playerCore.setPlaylist(payload.tracks, targetIdx, payload.meta);
          window.playerCore.setShuffle(!!window.shuffleMode);
          window.playerCore.setRepeat(!!window.repeatMode);
          // В "ИЗБРАННОМ" не нужен доп. фильтр - плейлист уже отфильтрован
          window.playerCore.setFavoritesOnly(false, []);
        }
      
        // Запуск воспроизведения
        window.playerCore.play(targetIdx);
      
        // Обновление UI
        if (payload?.tracks?.[targetIdx]?.lyrics) {
          loadLyrics(payload.tracks[targetIdx].lyrics);
        }
        ...
        // Обновление текущей строки в избранном
        if (viewMode === 'favorites') {
        ...
        window.__useNewPlayerCore = false;
      }
    }
    ...
    try {
      ...
      const tr = playingTracks[targetIdx];
      if (!tr) return;
      ...
      // Обновление UI
      if (tr.lyrics) loadLyrics(tr.lyrics);
