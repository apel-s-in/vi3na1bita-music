...
const validateRecommendationsAndStats = () => {
  ...
  contains('scripts/intel/recs/recommendation-engine.js', 'getRecommendationBehaviorSignals');
  contains('scripts/intel/recs/recommendation-engine.js', 'composeRecommendationScore');
  contains('scripts/intel/recs/recommendation-engine.js', 'source.canonicalByUid');
  contains('scripts/intel/recs/recommendation-score.js', 'confirmedAffinity');
  contains('scripts/intel/recs/recommendation-score.js', 'saturationPenalty');
  ...
  contains('scripts/app/gallery-recommendation-cards.js', '.filter(card => card.album || card.items.length)');
  contains('scripts/app/gallery-recommendation-cards.js', 'cardCache = { currentUid, builtAt: Date.now(), cards }');
  excludes('scripts/app/gallery-recommendation-cards.js', /emptyText|gallery-rec-card__empty/, 'Недостижимая пустая карточка Gallery снова добавлена');
  excludes('scripts/intel/recs/recommendation-reasons.js', /audio_similarity/, 'Неиспользуемая причина audio_similarity снова добавлена');
  excludes('styles/showcase.css', /\.gallery-rec-card__head>i\{/, 'Старая TEST-метка рекомендательной карточки снова добавлена');
  ...
  excludes('scripts/intel/recs/recommendation-engine.js', /profileCoverage|testProfile|test_fixture|preview\?\.\[key\]|contentWarnings/, 'Recommendation Engine снова использует тестовые или переходные TrackProfile');
  excludes('scripts/intel/listener/listener-profile.js', /preview\?\.\[key\]|preview\?\.preferences/, 'ListenerProfile снова поддерживает переходную форму TrackProfile');
  excludes('scripts/app/gallery-recommendation-cards.js', /preview\?\.\[key\]/, 'Gallery снова поддерживает переходную форму TrackProfile');
  ...
  contains('scripts/ci/validate-intel-profiles.mjs', 'track-profile-v4');
  contains('scripts/ci/validate-intel-profiles.mjs', 'track-profile-vocabulary-v2');
  ...
  contains('data/track-profiles/track-profile.template.json', '"technicalConfidence"');
  contains('data/track-profile-vocabulary.json', 'track-profile-vocabulary-v2');
  contains('data/track-profile-vocabulary.json', '"aliases"');
  ...
  contains('scripts/intel/track/track-similarity.js', 'loudnessLufs');
  contains('scripts/intel/track/track-similarity.js', 'dynamicRange');
  contains('scripts/intel/track/track-similarity.js', 'technicalConfidence');
  contains('scripts/intel/track/track-similarity.js', 'scoreTonalitySimilarity');
  contains('scripts/intel/track/track-similarity.js', 'TRACK_SIMILARITY_WEIGHTS');
  contains('scripts/intel/recs/recommendation-engine.js', 'sessionAffinity');
  contains('scripts/intel/recs/recommendation-engine.js', 'scoreTrackSimilarity');
  assert(!fs.existsSync(path.join(root, 'scripts/intel/track/track-presentation.js')), 'Лишний TrackPresentation adapter удалён');
  contains('scripts/ci/generate-track-profiles-index.mjs', 'track-profiles-index-v4');
  ...
  assert(!fs.existsSync(path.join(root, 'scripts/ci/generate-track-profile-review-context.mjs')), 'Review-context generator удалён');
  assert(!fs.existsSync(path.join(root, '.github/workflows/generate-track-profile-review.yml')), 'Review-context workflow удалён');
