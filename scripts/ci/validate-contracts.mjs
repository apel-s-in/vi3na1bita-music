...
const validatePlaybackOwnershipFoundation = () => {
  ...
  ['account_device_list', 'account_device_update', 'takeoverEnabled', 'remotePauseEnabled', 'alwaysConfirm'].forEach(marker => contains('scripts/app/profile/account-devices-view.js', marker));
...
const validateBackupProxy = () => {
  ...
  ['requestContext?.http?.method', 'requestContext?.httpMethod', "method === 'OPTIONS'", 'x-yandex-auth', 'upload_backup', 'upload_event_segment', 'archive_delete_segments'].forEach(marker => contains(proxy, marker));
  excludes(proxy, /backup_achievement_receipt|pending-backup-receipts|BACKUP_RECEIPT_SECRET|SIGNALING_URL/, 'Legacy backup reward transport остался в proxy');
