...
const albums = JSON.parse(
  fs.readFileSync('albums.json', 'utf8')
)?.albums || [];
...
const readAlbum = async album => {
  ...
  if (!response.ok) {
    throw new Error(
      `config_fetch_failed:${album.key}:${response.status}`
    );
  }
  ...
  return tracks.map(track => ({
    ...
    album: safe(album.key),
    albumTitle: safe(config.albumName || album.title),
    hiUrl: new URL(track.audio, baseUrl).toString(),
    loUrl: new URL(track.audio_low, baseUrl).toString()
  }));
};

const main = async () => {
  const albumTracks = await Promise.all(
    albums.map(readAlbum)
  );
  ...
  const duplicate = tracks.find(
    (track, index) =>
      tracks.findIndex(item => item.uid === track.uid) !== index
  );
  ...
  const rows = await mapLimit(
    ...
    async track => {
      if (!track.uid || !track.hiUrl || !track.loUrl) {
        throw new Error(
          `track_metadata_missing:${track.album}:${track.title}`
        );
      }
      ...
      const durationDifference = Math.abs(
        hi.duration - lo.duration
      );

      if (durationDifference > 1) {
        throw new Error(
          `duration_mismatch:${track.uid}:` +
          `${hi.duration}:${lo.duration}`
        );
      }
