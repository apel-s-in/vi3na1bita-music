...
const bannedKeys = new Set(['trackVersion', 'title', 'album', 'albumTitle', 'durationSec', 'source', 'analyzer', 'analyzedAt', 'generatedAt', 'updatedAt', 'provenance', 'verified', 'dynamicRangeLra', 'relations', 'embedding']);
...
if (template.version !== 'track-profile-v4' || template.uid !== '__TEMPLATE__' || template.status !== 'template') fail('Некорректный TrackProfile template v4');
if (taxonomy.taxonomyVersion !== 'taxonomy-v3') fail('Требуется taxonomy-v3');
if (vocabulary.version !== 'track-profile-vocabulary-v2') fail('Некорректный audio vocabulary');
if (index.version !== 'track-profiles-index-v4') fail('Требуется track-profiles-index-v4');
...
const aliasTargets = {
  instrumentation: vocabulary.instrumentation,
  vocalRoles: vocabulary.vocalRoles,
  vocalDelivery: vocabulary.vocalDelivery,
  arrangementTags: vocabulary.arrangementTags,
  productionTags: vocabulary.productionTags,
  keys: vocabulary.keys,
  timeSignatures: vocabulary.timeSignatures,
  languages: vocabulary.languages,
  narrativePerspectives: vocabulary.narrativePerspectives
};

Object.entries(vocabulary.aliases || {}).forEach(([group, aliases]) => {
  const allowed = new Set(Object.keys(aliasTargets[group] || {}));
  if (!object(aliases) || !allowed.size) fail(`Некорректная alias-группа ${group}`);
  Object.entries(aliases).forEach(([alias, target]) => {
    if (!safe(alias) || !allowed.has(safe(target))) fail(`Alias ${group}.${alias} ссылается на неизвестный ${target}`);
  });
});
...
for (const [uid, preview] of Object.entries(items)) {
  ...
  if (music.loudnessLufs != null && (!Number.isFinite(Number(music.loudnessLufs)) || Number(music.loudnessLufs) < -40 || Number(music.loudnessLufs) > 0)) fail(`${uid}: loudnessLufs вне диапазона -40..0`);
  if (music.dynamicRange != null && (!Number.isFinite(Number(music.dynamicRange)) || Number(music.dynamicRange) < 0 || Number(music.dynamicRange) > 30)) fail(`${uid}: dynamicRange вне диапазона 0..30`);
  if (!object(music.technicalConfidence)) fail(`${uid}: отсутствует technicalConfidence`);

  ['bpm', 'key', 'loudnessLufs', 'dynamicRange'].forEach(field => {
    const confidence = Number(music.technicalConfidence[field]);
    if (!Number.isFinite(confidence) || confidence < 0 || confidence > 1) fail(`${uid}: technicalConfidence.${field} вне 0..1`);
    if (music[field] == null && confidence !== 0) fail(`${uid}: technicalConfidence.${field} должен быть 0 для null`);
  });
...
console.log(`✅ Validated ${Object.keys(items).length} TrackProfiles v4`);
