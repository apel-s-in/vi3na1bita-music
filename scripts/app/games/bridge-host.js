...
import { getEmbeddedFriendsRpcMethod } from 'https://vi3na1bita.website.yandexcloud.net/Friends/embedded-rpc-contract.js?v=9.1.8';
const W = window;
...
export const createGameBridgeHost = ({ iframe, config = {}, onState } = {}) => {
  ...
  const handleSignalingRequest = async payload => {
    ...
    if (!requestId || !allowed?.has(action)) {
      send('GC_SIGNALING_RESPONSE', { requestId, ok: false, status: 403, error: 'game_rpc_action_forbidden' });
      return;
    }
    try {
      const data = normalizeRpcPayload(payload?.data);
      const result = await requestSocialAction(action, data);
      if (action === 'ranked_stake_prepare' || (action === 'ranked_match_status' && ['paid', 'refunded'].includes(result?.match?.economy?.status))) {
        W.ShardWallet?.refresh?.({ force: true }).catch(() => null);
      }
      send('GC_SIGNALING_RESPONSE', { requestId, ok: true, status: 200, result });
    } catch (error) {
      send('GC_SIGNALING_RESPONSE', { requestId, ok: false, status: Number(error?.status || 500), error: safe(error?.message || 'game_rpc_failed') });
    }
  };
  const onMessage = e => {
