...
(function PlayerUIModule() {
  ...
  // =========================
  // Small utils (единые, через window.Utils)
  // =========================
  const $ = (id) => w.Utils?.$(id) || document.getElementById(id);
  const on = (el, ev, fn, opts) => { if (el) el.addEventListener(ev, fn, opts); };
  const raf = (fn) => requestAnimationFrame(fn);

  const clamp = (n, a, b) => (w.Utils?.clamp ? w.Utils.clamp(n, a, b) : Math.max(a, Math.min(b, n)));
  const toInt = (v, d = 0) => (w.Utils?.toInt ? w.Utils.toInt(v, d) : (Number.isFinite(parseInt(String(v ?? ''), 10)) ? parseInt(String(v ?? ''), 10) : d));
  ...
  function createPlayerBlock() {
    ...
    if (!tpl || !('content' in tpl)) {
      console.error('❌ #player-template not found in index.html');
      // Fallback: чтобы не сломать приложение полностью, создадим пустой контейнер
      const fallback = document.createElement('div');
      fallback.className = 'lyrics-player-block';
      fallback.id = 'lyricsplayerblock';
      bindPlayerEvents(fallback);
      return fallback;
    }
    ...
    // ✅ Применяем актуальное состояние лирики так же, как раньше (lyricsViewMode + animationEnabled)
    const ls = w.LyricsController?.getState?.() || { lyricsViewMode: 'normal', animationEnabled: false };

    const lyricsWindow = block.querySelector('#lyrics-window');
    if (lyricsWindow) {
      lyricsWindow.classList.remove('lyrics-normal', 'lyrics-hidden', 'lyrics-expanded');
      lyricsWindow.classList.add(`lyrics-${ls.lyricsViewMode}`);
    }

    const bg = block.querySelector('.lyrics-animated-bg');
    if (bg) {
      bg.classList.toggle('active', !!ls.animationEnabled);
    }

    const lyricsToggle = block.querySelector('#lyrics-toggle-btn');
    if (lyricsToggle) {
      lyricsToggle.classList.remove('lyrics-normal', 'lyrics-hidden', 'lyrics-expanded');
      lyricsToggle.classList.add(`lyrics-${ls.lyricsViewMode}`);
    }
