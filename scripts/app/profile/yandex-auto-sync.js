...
const getLocalProfileSummary = async () => {
  let name = 'Слушатель';
  try {
    const { metaDB } = await import('../../analytics/meta-db.js');
    name = (await metaDB.getGlobal('user_profile').catch(() => null))?.value?.name || name;
  } catch {}
  return getLocalBackupUiSnapshot({ name });
};
...
// legacy helper intentionally kept removed:
// fresh-login restore now belongs to auth-onboarding-orchestrator.js only
...
const _checkCloudMetaOnly = async ({ isFreshLogin = false } = {}) => {
  ...
  try {
    ...
    const lS = await enrichLocalSummaryWithDb(await getLocalProfileSummary());
