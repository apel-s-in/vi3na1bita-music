...
async function applyPreloadedBackup({ backup, token, asNewDevice, profile, inheritDeviceKey = null, skipDeviceSettings = false }) {
  ...
  try {
    ...
    try { await new Promise(r => setTimeout(r, 100)); window.dispatchEvent(new CustomEvent('profile:data:refreshed', { detail: { reason: 'cloud_restore_final' } })); window.dispatchEvent(new CustomEvent('stats:updated', { detail: { source: 'cloud_restore' } })); window.dispatchEvent(new CustomEvent('achievements:updated', { detail: { total: window.achievementEngine?.achievements?.length || 0, unlocked: window.achievementEngine?.getCompletedCount?.() ?? 0, items: window.achievementEngine?.unlocked || {}, unlockMeta: window.achievementEngine?.unlockMeta || {}, profile: window.achievementEngine?.profile || { xp: 0, level: 1 }, source: 'cloud_restore' } })); window.dispatchEvent(new CustomEvent('favorites:updated', { detail: { source: 'cloud_restore' } })); window.dispatchEvent(new CustomEvent('devices:updated', { detail: { source: 'cloud_restore' } })); window.PlayerUI?.updateMiniHeader?.(); window.PlayerUI?.updatePlaylistFiltering?.(); } catch {}
    nSys?.success(`Прогресс восстановлен ✅${asNewDevice ? ' (новое устройство)' : ''}`);
  } catch (e) { const msg = String(e?.message || ''); if(msg.includes('local_data_owner_mismatch'))nSys?.error?.('На устройстве открыт локальный прогресс другого Яндекс-аккаунта. Restore остановлен без изменения данных.');else if(msg.includes('local_data_owner_confirmation_required'))nSys?.warning?.('Сначала подтвердите привязку локального прогресса к аккаунту.');else if(msg.includes('restore_owner_mismatch'))nSys?.error?.('Backup принадлежит другому Яндекс-аккаунту.');else if(msg.includes('backup_integrity_failed'))nSys?.error?.('Файл backup повреждён.');else nSys?.error?.('Ошибка применения backup: '+msg);throw e; }
}
