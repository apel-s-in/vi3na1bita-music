...
const renderEventRow = ev => {
  ...
  return `<div class="profile-list-item" data-event-domain="${esc(ev?.domain || '')}" data-event-type="${esc(ev?.type || '')}">
...
export const renderProfileLogs = async ({ container: c, metaDB: db }) => {
  ...
  const draw = async (filter = lg.dataset.filter || 'all') => {
    ...
    try {
      ...
      const list = raw.filter(ev => filter === 'all' || ev.domain === filter).slice(0, 100);
