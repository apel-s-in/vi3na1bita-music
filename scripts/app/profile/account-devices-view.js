...
const renderMiniRow = d => {
  ...
  return renderSmallListRow({ icon: icon(d), title: `${d.label || 'Устройство'}${isCur ? ' · это устройство' : ''}${retired ? ' · скрыто' : ''}`, desc: `${[d.class, d.browser, d.pwa ? 'PWA' : 'браузер'].filter(Boolean).join(' · ')} · ${fmt(d.lastSeenAt)}`, attrs: `data-dev-open="${esc(d.deviceStableId || d.deviceHash || '')}"`, style: `width:100%;text-align:left;cursor:pointer;opacity:${retired ? '.48' : '1'}` });
};

export const renderAccountDevicesBlock = () => {
  const r = rows(), active = r.filter(d => !Number(d.retiredAt || 0));
  return `<div class="yandex-auth-note" id="ya-devices-block" style="margin-top:8px"><div style="display:flex;align-items:center;justify-content:space-between;gap:10px;margin-bottom:8px"><div style="font-size:12px;font-weight:900;color:#eaf2ff;text-transform:uppercase;letter-spacing:.7px">📱 Устройства пользователя</div><button type="button" class="om-btn om-btn--ghost" id="ya-devices-refresh" style="font-size:11px;padding:5px 9px">↻</button></div><div style="font-size:11px;color:#7f93b5;margin-bottom:8px">Активных: ${active.length} · всего в истории: ${r.length}. Runtime playback хранится только на сервере и не входит в backup.</div><div style="display:flex;flex-direction:column;gap:6px">${r.length ? r.slice(0,3).map(renderMiniRow).join('') : '<div class="fav-empty">Устройства ещё не зарегистрированы</div>'}</div>${r.length > 3 ? `<button type="button" class="om-btn om-btn--outline" id="ya-devices-open" style="width:100%;margin-top:8px">Показать все устройства</button>` : ''}<button type="button" class="om-btn om-btn--ghost" id="ya-playback-diagnostics" style="width:100%;margin-top:8px">Диагностика playback</button></div>`;
};

const renderAuthHistory = d => {
  const a = Array.isArray(d.authHistory) ? d.authHistory : [];
  return a.length ? a.slice(0,10).map(x => renderSmallListRow({ title: [x.os, x.browser, x.pwa ? 'PWA' : 'браузер'].filter(Boolean).join(' · ') || 'Авторизация', desc: [fmt(x.ts), x.lang, x.timezone].filter(Boolean).join(' · '), style: 'padding:9px 10px' })).join('') : '<div class="fav-empty">История авторизаций пока пуста</div>';
};
...
const openDeviceModal = async ({ stableId = '', rerender } = {}) => {
  ...
  m?.addEventListener('click', async e => {
    const act = e.target.closest('[data-dev-act]')?.dataset.devAct; if (!act) return;
    if (act === 'rename') {
      window.Utils?.profileModals?.promptName?.({ title: 'Название устройства', value: d.label || '', btnText: 'Сохранить', onSubmit: val => { DeviceRegistry.saveDeviceRegistry(rows().map(x => (x.deviceStableId === d.deviceStableId || x.deviceHash === d.deviceHash) ? DeviceRegistry.normalizeDeviceRow({ ...x, label: val, retiredAt: 0, lastSeenAt: Date.now() }) : x)); if (isCur) localStorage.setItem('yandex:onboarding:device_label', val); markDevicesDirty({ immediate: true }); window.NotificationSystem?.success('Устройство переименовано'); m.remove(); rerender?.(); } });
    } else if (act === 'refresh') {
      await ensureCurrentDeviceRegistryRow({ label: d.label || '', authEvent: true }).catch(() => null); markDevicesDirty({ immediate: true }); window.NotificationSystem?.success('Устройство обновлено'); m.remove(); rerender?.();
    } else if (act === 'retire') {
      DeviceRegistry.saveDeviceRegistry(DeviceRegistry.retireDevicesInRegistry(rows(), [d])); markDevicesDirty({ immediate: true }); window.NotificationSystem?.success('Устройство скрыто из активных'); m.remove(); rerender?.();
    }
  });
};

export const bindAccountDevicesBlock = (root, rerender) => {
  const block = root?.querySelector('#ya-devices-block'); if (!block || block._bound) return; block._bound = true;
  block.addEventListener('click', async e => {
    ...
    if (e.target.closest('#ya-playback-diagnostics')) {
    ...
    if (e.target.closest('#ya-devices-refresh')) { await ensureCurrentDeviceRegistryRow({ authEvent: true }).catch(() => null); markDevicesDirty({ immediate: true }); window.NotificationSystem?.success('Список устройств обновлён'); rerender?.(); }
  });
};
