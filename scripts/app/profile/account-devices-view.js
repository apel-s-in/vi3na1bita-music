...
export const renderAccountDevicesBlock = () => {
  ...
  return `<section class="yandex-auth-note account-devices-card" id="ya-devices-block"><div class="account-card-heading"><span>📱 Playback и безопасность</span><button type="button" class="om-btn om-btn--ghost" id="ya-devices-refresh">↻</button></div><div class="account-device-summary">Активных устройств: <b>${active.length}</b> · серверный owner: <b>${esc(playbackOwner)}</b></div><div class="account-device-list">${state.loaded ? (state.items.length ? state.items.slice(0, 4).map(renderServerRow).join('') : '<div class="fav-empty">Серверные устройства не найдены</div>') : renderLoading()}</div>${state.items.length > 4 ? '<button type="button" class="om-btn om-btn--outline om-fullw" id="ya-devices-open">Показать все серверные устройства</button>' : ''}<div class="account-backup-device-note">История backup: ${backup.length} записей. Она используется только для восстановления и не управляет playback-доверием.</div><button type="button" class="om-btn om-btn--ghost om-fullw" id="ya-playback-diagnostics">Диагностика playback</button></section>`;
};
