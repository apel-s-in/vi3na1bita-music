...
function bindYandexActions(root, rerender) {
  ...
  // Обновляем sync-dot с учётом ready-статуса
  ...
  window.addEventListener('backup:sync:state', e => {
    const syncDot2 = root.querySelector('#ya-sync-dot'); if (!syncDot2) return;
    ...
    syncDot2.title = cfg.title;
    syncDot2.style.background = cfg.color;
    syncDot2.style.animation = cfg.anim ? 'syncPulse 1s infinite' : '';
