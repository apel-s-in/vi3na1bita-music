...
export const createProfileAchievementsView = ({ ctx, container: c, engine: e }) => {
  ...
  return {
    render: (f = 'available') => {
      ...
      c.innerHTML = it.length ? it.map(a => `<div class="ach-item ${a.isUnlocked ? 'done' : ''}" data-ach="${a.id}"><div class="ach-top"><div class="ach-title" style="color:${a.isUnlocked ? '#fff' : a.color || '#fff'}">${hEsc(a.icon)} ${hEsc(a.name)}</div></div><div class="ach-sub">${a.isUnlocked && a.unlockedAt ? unlockInfo(a) : a.isHidden ? 'Откроется при особых условиях' : hEsc(a.short)}</div>${!a.isUnlocked && !a.isHidden && a.progress ? `<div class="ach-progress"><div class="ach-mini-bar"><div class="ach-mini-fill" data-ach-fill="${a.id}" style="width:${a.progress.pct}%"></div></div></div>` : ''}<div class="ach-bottom"><div class="ach-reward">${rewardText(a)}</div><div class="ach-remaining" data-ach-remaining="${a.id}">${tH(a)}</div><button class="ach-more" type="button">Подробнее</button></div>${a.progressMeta?.serverConfirmed ? '<div class="ach-server-note">Подтверждено сервером · обновляется во время прослушивания</div>' : ''}<div class="ach-details" style="display:none"><div class="ach-details-title">Как выполнить</div><div class="ach-details-how">${hEsc(a.howTo || 'Выполните условия.')}</div>${a.desc ? `<div class="ach-details-desc">${hEsc(a.desc)}</div>` : ''}</div></div>`).join('') : '<div class="fav-empty">По данному фильтру ничего нет</div>';
    },
    updateLiveNodes: () => {
      if (!c || !eng()?.achievements) return;
