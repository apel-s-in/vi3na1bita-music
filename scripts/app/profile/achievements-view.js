...
export const createProfileAchievementsView = ({ ctx, container: c, engine: e }) => {
  const tM = ctx._achTimerMode ??= new Map(), esc = v => CSS.escape(String(v || ''));
  ...
  return {
    ...
    toggleTimerMode: id => { tM.set(id, gM(id) === 'remaining' ? 'elapsed' : 'remaining'); ctx._achCurrentFilter && flt(ctx._achCurrentFilter).length; const r = c?.querySelector(`[data-ach-remaining="${esc(id)}"]`); if (r) r.innerHTML = tH(e.achievements.find(a => a.id === id)); }
  };
};
