...
export async function openYandexRestoreFlow({ token, disk = YandexDisk, notify = window.NotificationSystem, rerender, localProfile } = {}) {
  ...
  const local = getLocalProfile(localProfile), localSummary = getLocalBackupUiSnapshot({ name: local?.name || 'Слушатель' }), cmp = compareLocalVsCloud(localSummary, meta);
    ...
    try {
      ...
      openRestorePreviewModal(data, async mode => {
        try {
          ...
          await refreshAfterRestore('cloud_restore');
          await markRestoreCompleted();
          notify?.success(`Восстановление завершено ✅ ${cmp?.state === 'cloud_richer_new_device' ? '(облако выглядело как источник для нового устройства)' : ''}`);
          rerender?.();
        } catch (e) {
  ...
  return { ok: true, compareState: cmp?.state || 'unknown', hasHistory: Array.isArray(items) && items.length > 0 };
}
