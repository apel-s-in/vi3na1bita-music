...
export const openRestorePreviewModal = (d, oC) => {
  ...
  const m = window.Modals?.open?.({
    ...
    maxWidth: 420,
    bodyHtml: `${renderStatusPill({ text: c.state || 'compare', tone: c.state === 'conflict' ? 'warn' : 'info' })}<div class="modal-confirm-text" style="font-size:13px;margin-top:10px"><b>Дата backup:</b> ${fmtDateTime(s.timestamp)}<br><b>Версия приложения:</b> ${esc(s.appVersion)}<br><b>Событий:</b> ${esc(s.eventCount || 0)}<br>${tH}<span style="color:#9db7dd">Сравнение: ${esc(cL)}. Restore объединяет данные через event log и безопасный merge.</span></div><div id="manual-restore-diff">${renderRestoreDiffHtml({ backup: d, localSummary: lI, cloudSummary: s })}</div>${renderModalNote({ html: '<b>Выберите режим восстановления:</b>', style: 'font-size:13px' })}${renderActionGrid([{ text:'Восстановить всё', primary:true, attrs:'data-restore-mode="all"' }, { text:'Профиль, избранное, плейлисты', attrs:'data-restore-mode="profile"' }, { text:'Статистику и достижения', attrs:'data-restore-mode="stats"' }])}`
  });
  if (!m) return;
  compareBackupBranches({ backup: d }).then(br => {
    const box = m.querySelector('#manual-restore-diff');
    if (box) box.innerHTML = renderRestoreDiffHtml({ backup: d, localSummary: lI, cloudSummary: s, branch: br });
  }).catch(() => {});
  m.addEventListener('click', e => {
    const b = e.target.closest('[data-restore-mode]');
    if (b) { m.remove(); oC?.(b.dataset.restoreMode || 'all'); }
  });
};
