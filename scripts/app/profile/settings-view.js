...
const readDeletedPlaylists = () => { try { return (JSON.parse(localStorage.getItem('sc3:playlists') || '[]') || []).filter(p => p?.id && Number(p.deletedAt || 0) > 0); } catch { return []; } };
const renderDataSection = () => {
  const del = readDeletedPlaylists();
  return `<div class="settings-content" id="set-data"><button type="button" class="set-acc-btn">КОРЗИНА ПЛЕЙЛИСТОВ</button><div class="set-acc-body">${del.length ? del.map(p => `<div class="set-row" style="gap:10px"><div class="set-info"><div class="set-title">🗑 ${String(p.name || 'Плейлист')}</div><div class="set-sub">Удалён: ${new Date(Number(p.deletedAt || 0)).toLocaleString('ru-RU')} · треков: ${(p.order || []).length}</div></div><button class="om-btn om-btn--ghost" data-pl-restore="${String(p.id)}">↩</button><button class="om-btn om-btn--danger" data-pl-purge="${String(p.id)}">×</button></div>`).join('') : '<div class="fav-empty">Удалённых плейлистов нет</div>'}</div></div>`;
};
...
export const renderProfileSettings = root => {
  if (!root) return;
  root.innerHTML = `<div class="ach-classic-tabs"><div class="ach-classic-tab active" data-set-tab="general">Общие</div><div class="ach-classic-tab" data-set-tab="interface">Интерфейс</div><div class="ach-classic-tab" data-set-tab="data">Данные</div><div class="ach-classic-tab" data-set-tab="benefits">Зачем вход?</div><div class="ach-classic-tab" data-set-tab="keys" id="set-tab-keys" style="display:none">Клавиатура</div></div>${renderGeneralSection()}${renderInterfaceSection()}${renderDataSection()}${renderBenefitsSection()}<div class="settings-content" id="set-keys"><div class="fav-empty">Раздел в разработке 🛠️</div></div>`;
