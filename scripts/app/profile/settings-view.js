...
export const renderProfileSettings = (root) => {
  ...
  root.innerHTML = `
    ...
    <div class="settings-content" id="set-interface">
      ...
      <div class="set-acc-body" id="set-pulse-body">
        ...
        <div class="set-row">
          <div class="set-info"><div class="set-title">Включить пульсацию</div><div class="set-sub">Локальная реакция логотипа на бас без сети и сценариев</div></div>
        ...
        <div class="set-row" style="flex-direction:column;align-items:stretch;gap:8px">
          <div class="set-info"><div class="set-title">Интенсивность</div><div class="set-sub">Мягкость масштаба и свечения логотипа</div></div>
  ...
  const lpEnabled = root.querySelector('#lp-enabled'), lpInt = root.querySelector('#lp-intensity'), lpReset = root.querySelector('#lp-reset-btn');
  if (lpEnabled && lpInt && lpReset) {
