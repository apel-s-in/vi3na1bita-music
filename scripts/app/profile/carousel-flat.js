...
const ensureStyles = () => {
  W.Utils?.dom?.createStyleOnce?.('sc-3d-carousel-flat-styles', `
    ...
    .sc-3d-btn{position:relative;background:linear-gradient(180deg,#0d1828,#070d16);border:1px solid rgba(77,170,255,.3);color:#7ab4f5;border-radius:14px;font-weight:900;cursor:pointer;transition:all .3s cubic-bezier(.4,0,.2,1);overflow:hidden;box-shadow:0 6px 15px rgba(0,0,0,.6)}
    .sc-3d-btn::before{content:'';position:absolute;top:0;left:0;right:0;height:45%;background:linear-gradient(180deg,rgba(255,255,255,.1),transparent);border-radius:14px 14px 0 0;pointer-events:none}
    ...
    .sc-3d-btn-sq{flex:0 0 52px;width:52px;height:52px;padding:0;font-size:18px;display:flex;align-items:center;justify-content:center}
    
    /* Центральная кнопка Выбрать */
    .sc-3d-btn-sel{flex:1;max-width:140px;height:52px;font-size:14px;text-transform:uppercase;letter-spacing:1px}
