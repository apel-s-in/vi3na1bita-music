  ...
  const renderAuthBlock = () => {
    ...
    authEl.innerHTML = `
      <div class="yandex-auth-block">
        ...
        ` : `
          <div style="color:#9db7dd;font-size:12px;line-height:1.5;margin-bottom:14px;word-break:break-word;white-space:normal">
            Войдите через Яндекс — прогресс, достижения и плейлисты сохранятся на всех устройствах.
          </div>
          <button data-ya-action="login" style="width:100%;padding:14px;font-size:15px;font-weight:900;border-radius:12px;border:none;cursor:pointer;background:linear-gradient(135deg,#cc0000,#880000);color:#fff;display:flex;align-items:center;justify-content:center;gap:10px;transition:opacity .2s">
            <span style="font-size:22px">Я</span> Войти через Яндекс
          </button>
        `}
      </div>`;
