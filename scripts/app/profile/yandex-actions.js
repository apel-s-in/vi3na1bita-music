...
import { renderSyncLogRow, renderCloudSectionCard, fmtDateTime, esc } from './cloud-card-renderers.js';
...
const openRecoverySnapshotModal = async () => {
  const { getRestoreRecoverySnapshotInfo, restoreRecoverySnapshot } = await import('../../analytics/backup-recovery.js');
  const info = await getRestoreRecoverySnapshotInfo();
  if (!info) return window.Modals?.open?.({ title: 'Recovery snapshot', maxWidth: 390, bodyHtml: '<div class="fav-empty">Локальный recovery snapshot пока не создан</div>' });

  window.Modals?.confirm?.({
    title: 'Последний локальный recovery snapshot',
    maxWidth: 430,
    textHtml: `${renderCloudSectionCard({ title: 'Сохранён перед restore', body: `<div class="modal-confirm-text" style="margin:0"><b>Дата:</b> ${fmtDateTime(info.savedAt)}<br><b>Причина:</b> ${esc(info.reason || 'before_restore')}</div>` })}<div style="font-size:12px;color:#9db7dd;line-height:1.45;margin-top:10px">Можно откатить локальное состояние к этому снимку. Воспроизведение напрямую не трогаем.</div>`,
    confirmText: 'Откатить',
    cancelText: 'Отмена',
    onConfirm: async () => {
      try {
        await restoreRecoverySnapshot({ BackupVault });
        window.NotificationSystem?.success?.('Recovery snapshot восстановлен ✅');
      } catch (e) {
        window.NotificationSystem?.error?.('Откат не удался: ' + String(e?.message || ''));
      }
    }
  });
};
...
export function initYandexActions() {
  window._handleYaAction = async (action, container, rerender) => {
    ...
    const acts = {
      ...
      'recovery-snapshot': () => openRecoverySnapshotModal(),
