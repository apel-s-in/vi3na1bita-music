  ...
  window._handleYaAction = async (action, container, rerender) => {
    ...
    if (action === 'restore-backup') {
      ...
      const runRestoreByPath = async selectedPath => {
        ...
        try {
          const data = await disk.download(token, selectedPath || undefined).catch(async e => {
            const msg = String(e?.message || '');
            ...
            if (msg.includes('disk_forbidden') || msg.includes('disk_auth_error') || msg.includes('token_invalid_or_expired')) {
              window.Modals?.confirm?.({
                ...
                textHtml: `${hint || 'Текущему токену не хватает прав на папку приложения Яндекс.Диска.'}<br><br>После выхода следующий вход запросит подтверждение заново.`,
                confirmText: 'Переподключить',
                cancelText: 'Отмена',
                onConfirm: () => {
                  ya.logout();
          ...
          if (String(e?.message || '').includes('download_cors_fallback_required')) {
            const href = e?.downloadHref || await disk.getDownloadLink(token, selectedPath || undefined).catch(() => null);
