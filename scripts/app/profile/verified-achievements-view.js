...
const render = st => {
  ...
  return `${renderCloudSectionCard({ title:'Проверенные достижения', body: `
    ${renderKeyValueRow({ label:'Локально открыто', value:st?.summary?.totalLocalUnlocked || 0, wordBreak:false })}
    ${renderKeyValueRow({ label:'Синхронизировано', value:st?.summary?.synced || 0, wordBreak:false })}
    ${renderKeyValueRow({ label:'Проверено сервером', value:st?.summary?.verified || 0, wordBreak:false })}
    ${renderKeyValueRow({ label:'Готово к claim', value:st?.summary?.claimable || 0, valueColor:'#81c784', wordBreak:false })}
    ${renderKeyValueRow({ label:'Нужна проверка', value:st?.summary?.suspicious || 0, valueColor:st?.summary?.suspicious ? '#ffb74d' : '#9db7dd', wordBreak:false })}
    ${renderKeyValueRow({ label:'Статус сервера', value:st?.serverOk ? 'доступен' : (st?.serverError || 'нет проверки'), wordBreak:false })}
  ` })}${renderCloudSectionCard({ title:'Готово к claim', style:'margin-top:10px', body: claim.length ? claim.map(row).join('') : '<div class="fav-empty">Пока нет claimable достижений</div>' })}${renderCloudSectionCard({ title:'Нужна проверка', style:'margin-top:10px', body: susp.length ? susp.map(row).join('') : '<div style="font-size:12px;color:#81c784">Подозрительных verified-флагов нет</div>' })}${renderCloudSectionCard({ title:'Проверено', style:'margin-top:10px', body: ver.length ? ver.slice(0, 8).map(row).join('') : '<div class="fav-empty">Проверенных достижений пока нет</div>' })}<div class="profile-inline-actions"><button class="modal-action-btn online" data-verified-refresh="1">Обновить verified</button></div><div style="margin-top:10px;color:#7f93b5;font-size:11px;line-height:1.45">Локальные достижения не блокируются. Server validation нужна только для внешних призов/claim.</div>`;
};
