...
export const openBackupFoundModal = m => {
  ...
  window.Modals?.open?.({ title: 'Облачная копия найдена', maxWidth: 400, bodyHtml: `${renderCloudSectionCard({ title: 'Облачная копия', body: `<div class="modal-confirm-text" style="margin:0">${renderStatusPill({ text: 'копия доступна', tone: 'ok' })}<br><br><b>Дата:</b> ${fmtDateTime(m?.timestamp)}<br><b>Профиль:</b> ${esc(m?.profileName || 'Слушатель')}<br><b>Устройство:</b> ${esc(dev || 'не указано')}<br><b>Версия приложения:</b> ${esc(m?.appVersion || 'unknown')}<br><b>Размер:</b> ${esc(m?.sizeHuman || 'unknown')}<br><b>Событий:</b> ${esc(m?.eventCount || 0)}<br><b>Сравнение:</b> ${esc(cL)}<br><b>Тип:</b> ${esc(c.state)}<br>${m?.historyPath ? `<b>История:</b> версионированный backup сохранён<br>` : ''}</div>` })}${renderModalNote('Копия хранится в личной папке приложения на Яндекс Диске и привязана к аккаунту владельца.')}` });
};
