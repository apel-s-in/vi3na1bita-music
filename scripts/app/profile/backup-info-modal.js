...
import { esc, fmtDateTime, renderCloudSectionCard } from './cloud-card-renderers.js';

export const openBackupInfoModal = () => window.Modals?.open?.({
  title: 'Что сохраняется в backup',
  maxWidth: 400,
  bodyHtml: `<div class="modal-confirm-text">Backup хранит общий прогресс аккаунта и данные синхронизации.</div>${renderCloudSectionCard({ title: 'Shared backup', body: `<ul style="margin:0 0 0 18px;color:#eaf2ff;line-height:1.5"><li>статистика и event log</li><li>достижения, XP, стрики</li><li>профиль</li><li>избранное и плейлисты</li><li>реестр устройств</li><li>внутренние intel/store данные</li></ul>` })}${renderCloudSectionCard({ title: 'Device settings', style: 'margin-top:10px', body: `<div style="color:#9db7dd;font-size:12px;line-height:1.45">Локальные настройки устройства сохраняются отдельно в <b>device-settings/&lt;deviceStableId&gt;.json</b>: громкость, качество, offline/UI/player prefs. Восстановление всегда выполняется вручную через предпросмотр и выбор устройства.</div>` })}`
});
