    ...
    achListEl.innerHTML = items.length ? items.map(a => {
      ...
      return `
        <div class="ach-item ${a.isUnlocked ? 'done' : ''}" data-ach="${a.id}">
          ...
          ${!a.isHidden ? `<div class="ach-details" style="display:none; grid-column: 1 / -1; padding:8px; border-top:1px dashed rgba(255,255,255,0.1); margin-top:6px;"><div style="color:#cfe3ff; font-weight:700; margin-bottom:6px;">Как выполнить</div><div style="color:#eaeffb; margin-bottom:6px; font-size:12px;">${a.howTo || 'Выполните условия.'}</div>${a.desc ? `<div style="color:#9aa8c4; font-size:.9em;">${a.desc}</div>` : ''}${p}</div>` : ''}
        </div>`;
