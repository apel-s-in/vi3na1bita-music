...
export async function loadProfileAlbum(ctx) {
  ...
  setTimeout(async () => {
    ...
    renderRecommendationsTab(container) {
      container.innerHTML = `
        <div class="profile-section fade-in">
          <h3 class="section-title">Специально для вас</h3>
          <div id="prof-recs-list" class="recs-grid">
      ...
      setTimeout(async () => {
        try {
          // Используем stats-aggregator для формирования умных рекомендаций
          const stats = await window.StatsAggregator.getGlobalStats();
          ...
          if (topGenres.length === 0) {
            html = `
              <div class="fav-empty">
                <i class="fas fa-headphones" style="font-size: 2rem; margin-bottom: 10px; opacity: 0.5;"></i><br>
                Слушайте больше треков, чтобы алгоритм подобрал рекомендации.
              </div>`;
          } else {
            html = topGenres.slice(0, 4).map(genre => `
              <div class="rec-card glass-panel">
                <div class="rec-icon"><i class="fas fa-magic"></i></div>
                <div class="rec-info">
                  <h4>Микс: ${genre.name || genre}</h4>
                  <p>На основе ваших вкусов</p>
                </div>
                <button class="btn-icon btn-play-rec" title="Слушать"><i class="fas fa-play"></i></button>
              </div>
            `).join('');
          }
          container.querySelector('#prof-recs-list').innerHTML = html;
        } catch (e) {
          console.error('[ProfileUI] Ошибка загрузки рекомендаций:', e);
          container.querySelector('#prof-recs-list').innerHTML = '<div class="fav-empty">Ошибка загрузки рекомендаций</div>';
        }
      }, 100);
    }

    // ------------------------------------------------------------------------
    // ЛОГИКА АВТОРИЗАЦИИ И СИНХРОНИЗАЦИИ (Cloud Sync & MetaDB)
    // ------------------------------------------------------------------------
    initAuthListeners() {
      // Кнопки OAuth
      document.getElementById('auth-yandex')?.addEventListener('click', () => this.handleAuth('yandex'));
      document.getElementById('auth-google')?.addEventListener('click', () => this.handleAuth('google'));
      document.getElementById('auth-vk')?.addEventListener('click', () => this.handleAuth('vk'));
      ...
      document.getElementById('sync-now-btn')?.addEventListener('click', async (e) => {
        ...
        try {
          if (window.CloudSync && typeof window.CloudSync.sync === 'function') {
            await window.CloudSync.sync();
          }
          window.EventLogger?.log('SYNC_MANUAL_TRIGGER', { status: 'success' });
          ...
          console.error('[ProfileUI] Ошибка синхронизации:', error);
    ...
    async handleAuth(provider) {
      window.EventLogger?.log('AUTH_ATTEMPT', { provider });
      try {
        // Интеграция с существующим cloud-sync.js
        if (provider === 'google' && window.CloudSync?.authenticateGoogle) {
          await window.CloudSync.authenticateGoogle();
        } else if (provider === 'yandex' && window.CloudSync?.authenticateYandex) {
          await window.CloudSync.authenticateYandex();
        } else {
          // Fallback / Мок для визуализации, если API еще пишется
          alert(`Процесс авторизации через ${provider.toUpperCase()} инициализирован.\nОкно провайдера откроется в новой вкладке.`);
        }
        
        window.EventLogger?.log('AUTH_SUCCESS', { provider });
        // Обновляем UI после успешной авторизации
        this.renderProfileHeader();
      } catch (e) {
        console.error(`[ProfileUI] Ошибка авторизации (${provider}):`, e);
        window.EventLogger?.log('AUTH_ERROR', { provider, error: e.message });
      }
    }
    ...
    // ------------------------------------------------------------------------
    editProfile() {
      // Используем session-tracker / meta-db для хранения
      const currentName = localStorage.getItem('app_user_name') || 'Слушатель';
      const newName = prompt('Введите ваше имя или никнейм:', currentName);
      
      if (newName && newName.trim() && newName.trim() !== currentName) {
        const cleanName = newName.trim().substring(0, 30);
        localStorage.setItem('app_user_name', cleanName);
        document.getElementById('profile-user-name').textContent = cleanName;
        window.EventLogger?.log('PROFILE_NAME_CHANGED');
      }
    }

    changeAvatar() {
      const avatars = ['🧑‍🚀', '🐱‍👤', '🎸', '🎧', '🦄', '😎', '🤖', '🦊', '🐼', '👽'];
      const current = localStorage.getItem('app_user_avatar') || '🧑‍🚀';
      const currentIndex = avatars.indexOf(current);
      const nextAvatar = avatars[(currentIndex + 1) % avatars.length];
      
      localStorage.setItem('app_user_avatar', nextAvatar);
      document.querySelector('.profile-avatar-img').textContent = nextAvatar;
      window.EventLogger?.log('PROFILE_AVATAR_CHANGED', { avatar: nextAvatar });
    }
}

// Экспортируем или инициализируем глобально
document.addEventListener('DOMContentLoaded', () => {
    window.UserProfile = new UserProfileUI();
});
