...
const $ = id => D.getElementById(id), esc = s => U.escapeHtml(String(s ?? '')), trk = u => W.TrackRegistry?.getTrackByUid?.(u);
const albT = k => W.TrackRegistry?.getAlbumTitle?.(k) || k || '', isDef = id => id === ALL, uidEsc = u => CSS.escape(String(u || ''));
...
const normSnap = (c, fbOrd = getCat()) => {
  ...
  const ord = [], hid = [], seen = new Set();
  (c.order || []).forEach(u => trk(u) && !seen.has(u) && (seen.add(u), ord.push(u)));
  fbOrd.forEach(u => trk(u) && !seen.has(u) && (seen.add(u), ord.push(u)));
  (c.hidden || []).forEach(u => trk(u) && seen.has(u) && !hid.includes(u) && hid.push(u));
...
const Store = {
  ...
  save: p => {
    const all = Store.pl(), i = all.findIndex(x => x.id === p.id), n = normCtx(p, false, p.order || getCat());
    i >= 0 ? all.splice(i, 1, n) : all.push(n);
    Store.setPl(all);
  },
...
class Draft {
  ...
  getList() {
    ...
    const cat = getCat(), seen = new Set(), out = [];
    this.ord.forEach(u => trk(u) && !seen.has(u) && (seen.add(u), out.push(u)));
    cat.forEach(u => trk(u) && !seen.has(u) && (seen.add(u), out.push(u)));
    return out;
  }
...
class ShowcaseManager {
  ...
  async _renderBody(tok) {
    ...
    const d = await this._disp(); if (tok !== this._tok) return;
  ...
  _renderNormal(box, { uids, hid }) {
    ...
    uids.forEach((u, i) => {
      ...
      if (ui.viewMode === 'grouped' && g !== t.sourceAlbum) { g = t.sourceAlbum; h += `<div class="showcase-group-header">── ${esc(albT(t.sourceAlbum))} ──</div>`; }
  ...
  _renderEdit(box, uids) {
    box.innerHTML = uids.map(u => {
      ...
      return `<div class="showcase-track sc-edit-row ${isH ? 'inactive' : ''} ${chk ? 'selected' : ''}" data-uid="${u}" data-hidden="${isH ? '1' : '0'}" draggable="true">
        <button class="sc-arrow-up">▲</button><div class="showcase-drag-handle">⠿</div><input type="checkbox" class="sc-chk" ${chk ? 'checked' : ''}>
        <img src="${t.cover}" class="showcase-track-thumb" loading="lazy">
        <div class="track-title"><div>${esc(t.title)}</div><div class="showcase-track-meta">${mt}</div></div>
        <button class="sc-eye-btn" title="Показать/Скрыть">${isH ? '🙈' : '👁'}</button><button class="sc-arrow-down">▼</button>
      </div>`;
  ...
  _renderPlaylists() {
    const a = $('sc-playlists-actions'), l = $('sc-playlists'); if (!a || !l) return;
    ...
    l.innerHTML = !pls.length ? '<div class="sc-pl-empty">Плейлистов пока('playlist') || v)).catch(() => W.NotificationSystem?.error('Ошибка буфера')),
  ...
  _saveEdit() {
    ...
    if (isDef(id)) {
      const c = Store.def(); c.order = ord; c.hidden = hid.filter(u => ord.includes(u)); Store.setDef(c);
    } else {
      const c = Store.get(id); if (!c) return;
      c.order = ord; c.hidden = hid.filter(u => ord.includes(u)); Store.save(c);
    }
    this._leaveEdit();
  ...
  _resetEdit() {
    ...
    W.Modals?.confirm({
      ...
      cancelText: 'Отмена',
      onConfirm: () => { this._drf.reset(); this._renderEdit($('sc-tracks-container'), this._drf.getList()); }
    });
  }
  ...
  _bindDrag(box) {
    if (box._scDrag) return; box._scDrag = 1;
  ...
  _openSort() {
    ...
    const opts = [['user','👤 Мой порядок'],['name-asc','А→Я'],['name-desc','Я→А'],['album-desc','Альбомы ↓'],['album-asc','Альбомы ↑'],['plays-desc','Топ прослушиванийURIComponent(escape(atob(String(b64).trim()))));
      if (!d?.n || !Array.isArray(d?.u)) throw 1;
      const u = d.u.filter(trk), miss = d.u.length - u.length;
      W.Modals?.confirm({ title: '🎵 Вам прислан плейлист', textHtml: `<b>${esc(d.n)}</b><br><br>Доступно треков: ${u.length} из ${d.u.length}.${miss > 0 ? '<br><span style="color:#ff9800">Часть треков недоступна.</span>' : ''}`, confirmText: 'Добавить', cancelText: 'Отмена', onConfirm: () => this._createPl(u, false, `${d.n} (Присланный)`) });
  ...
  openColorPicker(el, albumKey, playlistId) {
    ...
    m.onclick = e => {
      ...
      if (playlistId) {
        const p = Store.get(playlistId); if (p) { p.color = v; Store.save(p); this._renderPlaylists(); }
      } else if (aKey) {
        const c = Store.cols(); c[aKey] = v; Store.setCols(c); this._renderBody(++this._tok);
      }
      m.remove();
    };
  }
