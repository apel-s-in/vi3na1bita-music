...
class ShowcaseManager {
  ...
  async renderTab() {
    ...
    if (!isD) { const lu = localStorage.getItem('showcase:lastTrackUid'); if (lu && localStorage.getItem('showcase:lastPlayingContext') === (ctx.isPl ? `__showcase__:${ctx.id}` : '__showcase__')) this.hiTrack(lu); }
  }
  ...
  bindCtrl(root) {
    ...
    const acts = {
      ...
      'sc-b-add': () => { const c=this.getCtx(); if(c.isPl){ const p=this.state.pls.find(x=>x.id===c.id); [...this.searchSel].forEach(u=>{ if(!p.uids.includes(u)) p.uids.push(u); p.hid=p.hid.filter(x=>x!==u); }); p.ord=p.uids; } else { const d=this.state.def; [...this.searchSel].forEach(u=>{ if(!d.ord.includes(u)) d.ord.push(u); d.hid=d.hid.filter(x=>x!==u); }); } this.searchSel.clear(); this.searchQuery=''; this.saveState(); this.renderTab(); W.NotificationSystem.success('Добавлено'); },
  ...
  /* --- DRAFT ACTIONS --- */
  getFactoryBaseline() {
    let r = []; [...(W.albumsIndex || [])].reverse().forEach(a => { if (!a.key.startsWith('__')) W.TrackRegistry?.getAllUids()?.forEach(u => { if (W.TrackRegistry.getTrackByUid(u)?.sourceAlbum === a.key && !r.includes(u)) r.push(u); }); });
    return r;
  }

  enterEdit() {
    const c = this.getCtx();
    const ord = c.isPl ? [...c.ord] : this.getFactoryBaseline().map(u => c.ord.includes(u) ? null : u).filter(Boolean); // all tracks
    const realOrd = c.isPl ? [...c.ord] : [...c.ord, ...ord]; // existing + missing at end
    const chk = new Set(c.isPl ? c.uids : realOrd.filter(u => !c.hid.includes(u)));
    this.draft = { isPl: c.isPl, id: c.id, ord: realOrd, chk, hid: new Set(c.hid), dirty: false };
    this.renderTab();
  }
  ...
  saveEdit() {
    ...
    if (d.isPl) {
      ...
      if (p) { p.uids = d.ord.filter(u => d.chk.has(u)); p.ord = [...p.uids]; p.hid = p.uids.filter(u => d.hid.has(u)); }
    } else {
      this.state.def.ord = [...d.ord];
      this.state.def.hid = d.ord.filter(u => !d.chk.has(u)); // unchecked -> hidden
    }
    this.saveState(); this.draft = null; W.NotificationSystem.success('Сохранено'); this.renderTab();
  }
  ...
  resetEdit() {
    W.Modals.confirm({ title: 'Сбросить?', textHtml: 'Вернуть к начальному виду?', confirmText: 'Да', onConfirm: () => {
      if (this.draft.isPl) {
        const p = this.state.pls.find(x => x.id === this.draft.id);
        if (p && p.snap) { this.draft.ord = [...p.snap.ord]; this.draft.chk = new Set(p.snap.uids); this.draft.hid = new Set(p.snap.hid); }
      } else {
        const b = this.getFactoryBaseline();
        this.draft.ord = b; this.draft.chk = new Set(b); this.draft.hid = new Set();
      }
      this.draft.dirty = true; $('sc-reset').disabled = false; this.renderList();
    }});
  }
  ...
  async playCtx(uid = null) {
    ...
    if (this.searchQuery && uid) {
      if (!uids.includes(uid)) {
         ...
         W.playerCore.play(0); W.PlayerUI.ensurePlayerBlock(0, { userInitiated: true }); return;
      }
    }
  ...
  async renderList() {
    ...
    trks.forEach(({ uid, status, track: t }, i) => {
      ...
      h += `<div class="showcase-track ${eyeOff?'inactive':''} ${isS?'selected':''}" data-uid="${uid}" style="border-left: 3px solid ${cl}" ${isD?'draggable="true"':''}>${isD?`<button class="sc-arrow-up" data-dir="-1">▲</button>`:`<div class="tnum"${sN?'':' style="display:none"'}>${i+1}.</div>`}${isD||this.searchQuery?`<div class="showcase-drag-handle">${isD?'⠿':''}</div><div class="showcase-checkbox"></div>`:''}<img src="${this.getCover(t)}" class="showcase-track-thumb" loading="lazy"><div class="track-title"><div>${esc(t.title)}</div><div class="showcase-track-meta">${badges} ${esc(W.TrackRegistry.getAlbumTitle(t.sourceAlbum))}</div></div>${!isD?`<span class="offline-ind" data-uid="${uid}">🔒</span>`:''}${isD?`<button class="showcase-hide-btn">${eyeOff?'👁‍🗨':'👁'}</button>`:''}${!isD?`<img src="${W.playerCore?.isFavorite(uid)?'img/star.png':'img/star2.png'}" class="like-star" data-uid="${uid}" data-album="${t.sourceAlbum}"><button class="showcase-track-menu-btn">···</button>`:`<button class="sc-arrow-down" data-dir="1">▼</button>`}</div>`;
    });
  ...
  svOrd() { if(this.draft) { this.draft.ord = Array.from(D.querySelectorAll('.showcase-track')).map(e => e.dataset.uid); this.draft.dirty = true; $('sc-reset').disabled = false; } }
  strtDrg(e, n) {
  ...
  togHid(u) {
    ...
    if (!this.draft.isPl) { this.draft.hid.has(u) ? this.draft.chk.delete(u) : this.draft.chk.add(u); } // sync for default
    this.draft.dirty = true; $('sc-reset').disabled = false;
  ...
  togSel(u) {
    ...
    if (this.draft) { this.draft.dirty = true; $('sc-reset').disabled = false; if(!this.draft.isPl) { s.has(u) ? this.draft.hid.delete(u) : this.draft.hid.add(u); const b = el?.querySelector('.showcase-hide-btn'); if(b) b.textContent = s.has(u) ? '👁' : '👁‍🗨'; el?.classList.toggle('inactive', !s.has(u)); } }
