...
const getCat = () => (W.albumsIndex||[]).filter(a=>!a.key.startsWith('__')).flatMap(a=>(W.TrackRegistry?.getTracksForAlbum?.(a.key)||[]).map(t=>t.uid).filter(Boolean));

const normCtx = (c, isDef, fbOrd = getCat()) => {
  c = (c && typeof c === 'object') ? deep(c) : {};
  const ord = [], hid = [], oS = new Set(), fS = new Set((c.order || []).filter(trk));
  ...
  const res = { ...c, order: ord, hidden: hid, sortMode: c.sortMode || 'user', hiddenPlacement: c.hiddenPlacement || 'inline' };
  if (!isDef) res.creationSnapshot = c.creationSnapshot ? normCtx(c.creationSnapshot, false, ord) : { order: [...ord], hidden: [...hid] };
  return res;
};
...
const Store = {
  ...
  setPl: v => jSet('playlists', v.map(p => normCtx(p, false, p.order || getCat()))),
  ...
  def: () => normCtx(jGet('default'), true, getCat()), setDef: v => jSet('default', normCtx(v, true))
};
