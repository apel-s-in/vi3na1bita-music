...
class ShowcaseManager {
  ...
  _openMenu(uid, fromSrh = false) {
    ...
    bg.onclick = e => {
      const actId = e.target.id;
      if (e.target === bg || actId === 'bm-cx') return close();
      if (!actId) return;
      close();
      ...
      const acts = {
        'bm-play': () => {
          ...
          if (!one) return;
          this._playCtx(uid, false, [one], `__showcase__:search:${this._ctxId()}`);
        },
        'bm-pl': () => {
          setTimeout(() => {
            const pls = Store.pl();
            ...
            if (!m) return;
            m.onclick = ev => {
              const pick = ev.target.closest('[data-pid]');
              if (!pick) return;
              const p = Store.get(pick.dataset.pid);
              if (!p) return;
        ...
        'bm-dl': () => {
          ...
          U.download.applyDownloadLink(a, bldTrk(uid));
          if (a.href) a.click();
        },
