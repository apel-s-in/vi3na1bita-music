...
export const ensureLyricsIndexLoaded = async () => {
  ...
  return W.Utils?.func?.memoAsyncOnce?.('app:showcase:lyrics-index', async () => {
    try {
      const j = W.Utils?.fetchCache?.getJson ? await W.Utils.fetchCache.getJson({ key: 'lyrics:index:v1', url: I, ttlMs: 43200000, store: 'session', fetchInit: { cache: 'force-cache' } }) : await fetch(I, { cache: 'force-cache' }).then(r => { if (!r.ok) throw 1; return r.json(); });
      if (!j || j.v !== 1) throw 1;
