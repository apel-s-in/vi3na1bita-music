...
const currentOwner = () => safe(
  W.YandexAuth?.getProfile?.()?.yandexId ||
  W.YandexAuth?.getProfile?.()?.id
);

const currentDeviceId = () => safe(
...
const readIntent = owner => {
  try {
    const value = JSON.parse(
      localStorage.getItem(intentKey(owner)) || 'null'
    );
...
const instructionData = () => {
  ...
  if (type === 'ios') {
    return {
      ...
      steps: [
        'Откройте приложение именно в Safari.',
        'Нажмите кнопку «Поделиться» в нижней или верхней панели Safari.',
        'Прокрутите меню и выберите «На экран Домой».',
        'Подтвердите добавление кнопкой «Добавить».',
        'Закройте Safari и запустите приложение новым ярлыком с экрана Домой.'
      ],
  ...
  if (type === 'android') {
    return {
      ...
      steps: [
        ...
        'Запустите приложение созданным ярлыком с главного экрана.'
      ],
  ...
  return {
    ...
    steps: [
      ...
      'Выберите «Установить Витрина Разбита» или «Установить приложение».',
      'Подтвердите установку.',
      'Закройте эту вкладку и запустите приложение ярлыком с рабочего стола или из меню приложений.'
    ],
...
const beginInstallIntent = async () => {
  ...
  if (
    W.YandexAuth?.getSessionStatus?.() !== 'active' ||
    !W.YandexAuth?.isTokenAlive?.() ||
    !owner
  ) {
    W.Modals?.choice?.({
      title: 'Нужен вход через Яндекс',
      textHtml:
        'Для подтверждения PWA-достижения сначала войдите через Яндекс.<br><br>' +
        'Установка и награда будут привязаны к одному Яндекс ID.',
      actions: [
        {
          key: 'login',
          text: 'Войти через Яндекс',
          primary: true,
          onClick: () => W.YandexAuth?.login?.()
        },
        {
          key: 'cancel',
          text: 'Позже',
  ...
  if (
    ...
    writeIntent(owner, {
      intentId: safe(result.intentId),
      challenge: safe(result.challenge),
      deviceId: currentDeviceId(),
      platform: platform(),
      createdAt: Date.now(),
...
const verifyStandaloneLaunch = async () => {
  ...
  verificationPromise = requestSocialAction(
    'pwa_launch_verify',
    {
    ...
    .then(result => {
      ...
      W.NotificationSystem?.success?.(
        result?.duplicate
          ? 'PWA уже подтверждено для вашего аккаунта'
          : '📱 Приложение установлено. Достижение подтверждено!'
      );
  ...
  try {
    ...
    if (intent.alreadyCompleted) {
      W.NotificationSystem?.info?.(
        'PWA-достижение уже выполнено для этого Яндекс ID'
      );
    }
    ...
    if (deferredPrompt) {
      ...
      if (choice?.outcome === 'accepted') {
        W.NotificationSystem?.success?.(
          'Установка началась. После неё запустите приложение с ярлыка.'
        );
      } else {
        W.NotificationSystem?.info?.(
          'Установка отменена. Можно повторить позже.'
        );
      }
    ...
    W.NotificationSystem?.error?.(
      `Не удалось начать установку: ${error?.message || 'ошибка'}`
    );
  } finally {
...
export const initPwaInstall = () => {
  ...
  W.addEventListener('appinstalled', () => {
    ...
    W.NotificationSystem?.success?.(
      'Приложение установлено. Запустите его с нового ярлыка.'
    );
  });
