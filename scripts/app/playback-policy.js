    ...
    apply: () => {
      ...
      if (needFilter) {
        ...
        if (!target.length && !isFavAlbum) {
          ...
          // FIX: Просто выходим, не меняя плейлист и не ломая LS. 
          // Кнопка в UI останется нажатой (если юзер нажал), но фильтрация не пройдет.
          // Это лучше, чем рекурсивные вызовы или скрытый сброс.
          return; 
        }
      }
