    ...
    apply: () => {
      ...
      if (needFilter) {
        ...
        if (!target.length && !isFavAlbum) {
          // Не сбрасываем флаг тихо, просто уведомляем и не меняем плейлист
          W.NotificationSystem?.info('Нет избранных треков для фильтрации');
          // Возвращаемся к полному списку, но флаг в UI можно оставить или выключить явно
          // В данном случае безопаснее ничего не делать с плеером, 
          // либо выключить режим F без рекурсии:
