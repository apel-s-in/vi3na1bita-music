    ...
    apply: () => {
      ...
      if (needFilter) {
        ...
        if (!target.length && !isFavAlbum) {
          W.NotificationSystem?.info('Нет избранных треков. Играем всё.');
          // FIX: Не меняем localStorage и не трогаем кнопку F автоматически.
          // Просто не применяем фильтр к плейлисту в этот раз.
          return; 
        }
      }
