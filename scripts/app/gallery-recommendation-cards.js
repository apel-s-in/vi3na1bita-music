...
import { resolveRecommendationDataSource } from '../intel/recs/recommendation-data-source.js';
...
const CARD_CACHE_MS = 60 * 1000;
let cardCache = null;

const invalidateCardCache = () => {
  cardCache = null;
};
['stats:updated', 'stats:rebuilt', 'favorites:updated', 'intel:listener-profile:updated', 'intel:track-profiles:index-ready', 'account:data-switching'].forEach(name => window.addEventListener(name, invalidateCardCache));
...
const cardItems = (items, limit = 3) => items.slice(0, limit).map(item => {
  ...
  return {
    uid: item.uid,
    title: row?.title || item.uid,
    albumKey: row?.sourceAlbum || '',
    albumTitle: albumTitle(row?.sourceAlbum || ''),
...
const albumOfWeek = source => {
  ...
  const scores = albums.map(album => ({
    ...album,
    seconds: (window.TrackRegistry?.getTracksForAlbum?.(album.key) || []).reduce((sum, row) => sum + num(source.canonicalByUid.get(row.uid)?.globalListenSeconds), 0)
  }));
...
export const buildGalleryRecommendationCards = async () => {
  ...
  if (cardCache && cardCache.currentUid === currentUid && Date.now() - cardCache.builtAt < CARD_CACHE_MS) return cardCache.cards;
  ...
  const source = resolveRecommendationDataSource(localRows);
  const previews = previewItems(index);

  const forgotten = [...source.localByUid.entries()]
  ...
  const unfinished = [...source.localByUid.entries()]
    .map(([uid, row]) => ({ uid, preview: index?.items?.[uid] || null, row }))
    .filter(item => {
