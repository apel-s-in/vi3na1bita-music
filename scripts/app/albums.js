...
class AlbumsManager {
  ...
  _bindEvents() {
    ...
    $('track-list')?.addEventListener('click', (e) => {
      ...
      // В режиме Избранного клики обрабатывает favorites-view.js
      if (this.curr === FAV) return;

      const trk = e.target.closest('.track');
      if (!trk) return;

      // Клик по офлайн-индикатору обрабатывается в offline-indicators.js (stopPropagation)
  ...
  async _loadReg(key) {
    ...
    // Вставить офлайн-индикаторы 🔒/☁ во все треки
    this._injectOfflineIndicators();
    
    window.PlayerUI?.updateMiniHeader?.();
  }

  /** Проставить 🔒/☁ индикаторы во все .track[data-uid] в текущем track-list */
  _injectOfflineIndicators() {
