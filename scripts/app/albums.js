...
class AlbumsManager {
  ...
  _renderIcons() {
    ...
    box.innerHTML = (C.ICON_ALBUMS_ORDER || []).filter(it => it.key && (it.key.startsWith('__') || idx.some(a => a.key === it.key))).map(it => {
      if (it.key === PROFILE) return `<div class="album-icon profile-dyn-icon" data-album="${it.key}" data-akey="${it.key}" title="${escHtml(it.title)}"><span class="pg-lvl-val" id="pg-lvl-val">-</span><div class="pg-xp-cur" id="pg-xp-cur">...</div></div>`;
