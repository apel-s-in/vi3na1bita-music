...
const W = window, D = document, C = W.APP_CONFIG || {}, { $, toStr, escHtml, isMobileUA } = W.AppUtils || { $: id => D.getElementById(id), toStr: v => v == null ? '' : String(v), escHtml: s => String(s||''), isMobileUA: () => false };
...
class AlbumsManager {
  ...
  _bindEvents() {
    ...
    $('track-list')?.addEventListener('click', e => {
      ...
      const afterPlay = makeFavoritesOnlyAfterPlay({ highlight: (i, meta) => this.highlightCurrentTrack(i, meta), ensureBlock: (i, o) => W.PlayerUI?.ensurePlayerBlock?.(i, o) });
      return playWithFavoritesOnlyResolution({
        list: data._pTracks, uid, albumKey: aKey, track: data._pTracks[pIdx],
  ...
  async loadAlbum(key) {
    ...
    this.loading = true; this.galVis = true;
    try {
      ...
      if (sp[key]) {
        const mod = await import('./albums/specials.js'), fn = mod?.[sp[key]];
        ...
        const cSlot = $('cover-slot'), coverWrap = $('cover-wrap');
      ...
      console.error('[AlbumsManager] Ошибка:', e); W.NotificationSystem?.error('Ошибка загрузки');
    } finally { this.loading = false; }
  }

  highlightCurrentTrack() {
    D.querySelectorAll('.current').forEach(n => { if (['track','showcase-track','profile-list-item','sm-top-row'].some(c => n.classList.contains(c)) || n.tagName === 'LI') n.classList.remove('current'); });
    const u = W.playerCore?.getCurrentTrackUid?.();
    if (u) D.querySelectorAll(`[data-uid="${CSS.escape(u)}"]`).forEach(el => { if (['track','showcase-track','profile-list-item','sm-top-row'].some(c => el.classList.contains(c)) || el.tagName === 'LI') el.classList.add('current'); });
  }
