...
const LS_SHARED_HASH = 'backup:last_shared_semantic_hash:v1', LS_DEVICE_HASH_PREFIX = 'backup:last_device_settings_hash:v1:', LS_LOCAL_SUMMARY = 'backup:last_local_summary:v1', CLOUD_EVENT_TAIL_LIMIT = 50, CLOUD_UPLOAD_BODY_BUDGET_BYTES = PROXY_UPLOAD_MAX_BYTES - 65536, CLOUD_BACKUP_PATH = 'app:/Backup/vi3na1bita_backup.vi3bak';
...
export const buildSharedSemanticPayload = backup => ({
  version: sS(backup?.version || backup?.revision?.version || '6.1'),
  ...
  data: {
    stats: normalizeStatsForHash(backup?.data?.stats),
    eventLog: { warm: (Array.isArray(backup?.data?.eventLog?.warm) ? backup.data.eventLog.warm : []).filter(item => item?.eventId && !isBackupSemanticNoiseEvent(item)) },
    userProfile: backup?.data?.userProfile || {},
    localStorage: getSharedSnapshotLocalEntries(backup?.data?.localStorage || {}),
    intel: backup?.data?.intel || {}
  }
});
...
const compactBackupForCloud = async (b, { disk = null, token = '' } = {}) => {
  ...
  if (!coverage.size) {
    ...
    throw new Error(`backup_too_large_without_archive:${beforeBytes}:${CLOUD_UPLOAD_BODY_BUDGET_BYTES}`);
  }
...
const runUploadBackupBundle = async ({ disk, token, BackupVault = DefaultBackupVault, backup = null, force = false, uploadDevice = true, reason = 'autosave', syncLease = null } = {}) => {
  ...
  if (shouldUploadShared) {
    ...
    const writeHistory = false;

    if (typeof disk.uploadEventSegment === 'function') {
      eventArchive = await uploadLocalEventArchiveUntilCaughtUp({ disk, token, maxSegments: 1 }).catch(error => ({ ok: false, uploaded: false, reason: error?.message || 'event_archive_failed' }));
