...
const normAuthHistory = a => (Array.isArray(a) ? a : []).map(x => ({
  ts: safeNum(x?.ts || x?.at),
  browser: safeString(x?.browser || ''),
  os: safeString(x?.os || ''),
  lang: safeString(x?.lang || ''),
  timezone: safeString(x?.timezone || ''),
  pwa: !!x?.pwa
})).filter(x => x.ts > 0).sort((a,b)=>b.ts-a.ts).slice(0, 20);
...
export const normalizeDeviceRegistry = rows => {
  ...
  normalized.forEach(r => {
    ...
    const merged = normalizeDeviceRow({
      ...
      retiredAt: Math.max(safeNum(existing.retiredAt), safeNum(r.retiredAt)),
      authHistory: normAuthHistory([...(existing.authHistory || []), ...(r.authHistory || [])]),
      seenHashes: [
        ...new Set([
          ...(existing.seenHashes || []),
          ...(r.seenHashes || []),
...
export const retireDevicesInRegistry = (r, sR, ts = Date.now()) => {
  const sD=normalizeDeviceRegistry(sR), stI=new Set(sD.map(d=>safeString(d?.deviceStableId)).filter(Boolean)), hsI=new Set(sD.map(d=>safeString(d?.deviceHash)).filter(Boolean));
  return normalizeDeviceRegistry(r).map(d => (stI.has(safeString(d?.deviceStableId)) || (!safeString(d?.deviceStableId) && hsI.has(safeString(d?.deviceHash)))) ? normalizeDeviceRow({ ...d, retiredAt: safeNum(ts) || Date.now(), lastSeenAt: Math.max(safeNum(d.lastSeenAt), safeNum(ts)) }) : d);
};

const DeviceRegistry = { getDeviceRegistry, normalizeDeviceRow, normalizeDeviceRegistry, saveDeviceRegistry, getCurrentDeviceIdentity, isSameDevice, isCurrentDevice, getOtherDevices, countDeviceStableIds, removeDevicesFromRegistry, retireDevicesInRegistry };
