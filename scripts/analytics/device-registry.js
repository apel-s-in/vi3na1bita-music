...
export const normalizeDeviceRow = (r = {}) => { const sH=Array.isArray(r?.seenHashes)?[...new Set(r.seenHashes.map(safeString).filter(Boolean))]:[], dH=safeString(r?.deviceHash||''); if(dH&&!sH.includes(dH)) sH.push(dH); return { ...r, deviceHash:dH, deviceStableId:safeString(r?.deviceStableId||''), platform:safeString(r?.platform||'web')||'web', userAgent:safeString(r?.userAgent||''), firstSeenAt:safeNum(r?.firstSeenAt), lastSeenAt:safeNum(r?.lastSeenAt), seenHashes:sH }; };
