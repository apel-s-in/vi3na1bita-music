// UID.069_(Internal user identity)_(не смешивать provider token и identity модели)_(future cloud sync должен работать поверх provider identity layer) // UID.073_(Hybrid sync orchestrator)_(дать primary/mirror/social роли провайдеров)_(этот файл законсервирован как legacy-reference transport и не должен использоваться активным runtime) // UID.074_(Primary backup provider)_(готовить Yandex как основной backup channel)_(активный runtime уже работает через scripts/core/yandex-auth.js + scripts/core/yandex-disk.js + scripts/analytics/backup-vault.js) // UID.075_(Secondary mirror backup)_(подготовить Google как резервный mirror)_(эту заготовку можно переиспользовать позже при реальном multi-provider sync)
