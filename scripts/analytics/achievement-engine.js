...
export class AchievementEngine {
  ...
  async check({ force = false } = {}) {
    ...
    for (const [k, r] of Object.entries(this.dict)) {
      ...
      if (r.type === 'static' && !this.unlocked[k]) {
        if (r.trigger.conditions.every(c => this._evalCondition({ ...c, target: c.target }, agg))) { this.unlocked[k] = now; eXp += r.reward.xp; chg = true; this._notifyUnlock(r.ui.name, r.ui.icon, r.reward.xp, k); }
        ...
        while (sft-- > 0 && (!r.scaling.maxLevel || lvl <= r.scaling.maxLevel) && (!r.scaling.steps || lvl <= r.scaling.steps.length)) {
          if (r.trigger.conditions.every(c => this._evalCondition({ ...c, target: this._getSc(r, lvl, false) }, agg))) { this.unlocked[`${k}_${lvl}`] = now; const xp = this._getSc(r, lvl, true); eXp += xp; chg = true; this._notifyUnlock(r.ui.name.replace('{level}', lvl), r.ui.icon, xp, `${k}_${lvl}`); lvl++; } else break;
        }
      }
    }
  ...
  _buildUIArray() {
    ...
    const add = (id, r, lvl, unl, uAt, c, t) => {
      ...
      arr.push({ id, name: lvl ? r.ui.name.replace('{level}', lvl) : (isH ? 'Секретное достижение' : r.ui.name), short: isH ? 'Откроется при особых условиях' : r.ui.short.replace(/{target[a-z_]*}/g, vT), desc: isH ? 'Продолжайте исследовать приложение, чтобы узнать секрет.' : r.ui.desc, howTo: isH ? 'Скрыто' : r.ui.howTo, icon: isH ? '🔒' : r.ui.icon, color: isH || (!unl && lvl) ? '#888888' : r.ui.color, isUnlocked: unl, isHidden: isH, isSecret: !!r.hidden || r.category === 'secret', unlockedAt: uAt || null, xpReward: lvl ? this._getSc(r, lvl, true) : (r.reward.xp || 0), ...(!unl && !isH && rT > 0 && { progress: { current: vC, target: vT, pct } }), ...(pM && { progressMeta: pM }) });
    };
    for (const [k, r] of Object.entries(this.dict)) {
  ...
  _notifyUnlock(name, icon, xp, id = '') { eventLogger.log('ACHIEVEMENT_UNLOCK', null, { id, name, icon, xp }); if (!this._silentNotify) window.NotificationSystem?.success(`🏆 ${icon} Открыто: ${name} (+${xp} XP)`); }
