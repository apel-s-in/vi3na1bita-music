...
export class AchievementEngine {
  ...
  _generateAchievements() {
    const list = [];
    // Первая кровь
    list.push({ id: 'first_blood', name: 'Первая кровь', desc: 'Первое полное прослушивание', check: (stats) => stats.globalFullListens?.value >= 1 });
    // Генерация 50 уровней Аудиофила
    for(let i=1; i<=50; i++) {
       list.push({ id: `audiophile_${i}`, name: `Аудиофил ур. ${i}`, desc: `${i * 10} прослушиваний`, check: (stats) => stats.globalFullListens?.value >= (i * 10) });
    }
    // Генерация 30 уровней Стриков
    for(let i=1; i<=30; i++) {
       list.push({ id: `streak_${i}`, name: `Стабильность ур. ${i}`, desc: `${i * 5} дней подряд`, check: (stats) => stats.currentStreak?.value >= (i * 5) });
    }
    // Спец. ачивки
  ...
  async checkAchievements() {
    ...
    // Подтягиваем текущие агрегаты в память для проверки
    const currentStats = {
    ...
    if (newlyUnlocked > 0) {
      await metaDB.updateStat('unlocked_achievements', (stat) => {
        stat.details = unlocked;
        stat.value = Object.keys(unlocked).length;
        return stat;
      });
  ...
  showNotification(ach) {
    // Вызов встроенной системы уведомлений приложения
    const event = new CustomEvent('app:notify', { detail: { title: 'Достижение разблокировано!', message: ach.name, type: 'achievement' } });
