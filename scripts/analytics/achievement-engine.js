    ...
    for (const [k, r] of Object.entries(this.dict)) {
      ...
      if (r.type === 'static' && !this.unlocked[k]) {
        ...
        while (sft-- > 0 && (!r.scaling.maxLevel || lvl <= r.scaling.maxLevel) && (!r.scaling.steps || lvl <= r.scaling.steps.length)) {
          if (r.trigger.conditions.every(c => this._evalCondition({ ...c, target: this._getSc(r, lvl, false) }, agg))) { this.unlocked[`${k}_${lvl}`] = now; const xp = this._getSc(r, lvl, true); eXp += xp; chg = true; this._notifyUnlock(r.ui.name.replace('{level}', lvl), r.ui.icon, xp); lvl++; } else break;
        }
      }
    }
