...
import {
  applyAccountCachePolicies,
  exportAccountCachePolicies
} from '../offline/cache-db.js';
...
const STORAGE_PREFIXES = Object.freeze([
  'gc_data_',
  'backup:last_device_settings_hash:v1:',
  'backup:event_archive:last_seq:v2:',
  'backup:event_archive:last_hash:v2:',
  'vf_webpush_sync_'
]);
...
const currentYandexId = () =>
  safe(window.YandexAuth?.getProfile?.()?.yandexId);

const isAccountStorageKey = key =>
  EXACT_STORAGE_KEYS.has(key) ||
...
const ownerStorageKey = async owner => {
  const bytes = await crypto.subtle.digest(
    'SHA-256',
    new TextEncoder().encode(`account:${safe(owner)}`)
  );
...
const vaultPut = async (owner, snapshot) => {
  ...
  return new Promise((resolve, reject) => {
    ...
    tx.objectStore(VAULT_STORE).put({
      key,
      ownerYandexId: owner === LOCAL_OWNER ? '' : owner,
      updatedAt: Date.now(),
      snapshot
    });
...
const captureCurrentSnapshot = async () => ({
  version: 1,
  savedAt: Date.now(),
  metaDB: await metaDB.exportSnapshot(),
  localStorage: collectAccountStorage(),
  cachePolicies: await exportAccountCachePolicies()
    .catch(() => ({}))
});
...
const applySnapshot = async snapshot => {
  ...
  await applyAccountCachePolicies(
    data.cachePolicies || {}
  ).catch(() => null);
};
...
const askToAdoptLocalData = owner =>
  new Promise(resolve => {
    ...
    window.Modals.choice({
      ...
      textHtml:
        'На устройстве есть прогресс без привязки к аккаунту.<br><br>' +
        'Перенести его в текущий Яндекс-аккаунт? ' +
        'Кэш музыки останется общим в любом случае.',
      actions: [
        {
          key: 'adopt',
...
const reloadAccountRuntime = async reason => {
  ...
  window.OfflineIndicators?.refreshAllIndicators?.()
    .catch(() => null);
  ...
  if (
    current &&
    !['__games__', '__friends__'].includes(current)
  ) {
...
const performSwitch = async targetOwner => {
  ...
  try {
    await import('./backup-sync-engine.js')
      .then(module =>
        module.suspendSyncForAccountSwitch?.()
      )
      .catch(() => null);
...
export const initAccountDataBoundary = async () => {
  ...
  window.addEventListener(
    'yandex:auth:changed',
    event => {
      const owner = safe(event.detail?.profile?.yandexId) ||
        currentYandexId();
      ...
      if (
        ...
        AccountDataContext
          .switchToYandexAccount(owner)
          .catch(error =>
            window.NotificationSystem?.error?.(
              `Не удалось переключить профиль: ${error?.message || 'ошибка'}`
            )
          );
      } else if (
        event.detail?.status === 'logged_out'
      ) {
        AccountDataContext
          .switchToLocal()
          .catch(() => null);
