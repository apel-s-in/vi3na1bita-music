...
const readCachedCloudMeta = (maxAgeMs = 10 * 60000) => {
  try {
...
const checkSafe = async (d, t) => { try { const cached=readCachedCloudMeta(), cm=cached||await d.getMeta(t).catch(()=>null); if(cm&&!cached) writeCachedCloudMeta(cm); const lSum=await enrich(getLocSum()), cmp=compareLocalVsCloud(lSum, cm); if(cmp.state==='no_cloud') return{ok:true,reason:'no_cloud'}; if(['local_richer','local_probably_richer','equivalent'].includes(cmp.state)) return{ok:true,reason:cmp.state,compare:cmp,cloudMeta:cm}; return{ok:false,reason:cmp.state,compare:cmp,cloudMeta:cm}; } catch(e){ const m=String(e?.message||''); if(m.includes('disk_forbidden')||m.includes('disk_auth_error')||m.includes('403')){console.warn('[BackupSyncEngine] disk forbidden, blocking autosave until reauth');return{ok:false,reason:'cloud_forbidden',error:m};} return{ok:false,reason:'cloud_compare_failed',error:m}; } };
...
export const initBackupSyncEngine = () => {
  ...
  window.addEventListener('backup:domain-dirty', e => {
    if(!_rdy) return;
    markDirty({ immediate:!!e.detail?.immediate, domain:e.detail?.domain||'generic' });
  });
