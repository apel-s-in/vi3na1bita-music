...
import { getBackupResourceBusyReason, withBackupCoordinatorLease } from './backup-coordinator-client.js';
import {
...
const runDueSyncImpl = async ({ reason = 'scheduled_daily', force = false } = {}) => {
  ...
  try {
    ...
    if (!coordinated.granted) {
      const retryAt = Number(coordinated.retryAt || Date.now() + BACKUP_QUIET_RETRY_MS);
      await persistState({
        queue: coordinated.coordinator || null,
        nextSyncAt: retryAt,
        continuationAt: retryAt,
        deferredReason: coordinated.queued
          ? 'coordinator_queued'
          : coordinated.blocked
            ? 'coordinator_blocked'
            : coordinated.reason || 'coordinator_busy'
      });
      scheduleAt(retryAt);
      return false;
    }
