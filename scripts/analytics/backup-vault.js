...
async function readDeviceRegistry() {
  ...
  rows.forEach(x => {
    const key = String(x?.deviceStableId || '').trim() || String(x?.deviceHash || '').trim();
    if (!key) return;
    ...
    byStable.set(key, {
      ...prev,
      ...x,
      firstSeenAt: minPositive(prev.firstSeenAt, x.firstSeenAt) || Date.now(),
      lastSeenAt: Math.max(toNum(prev.lastSeenAt), toNum(x.lastSeenAt)),
      seenHashes: [...new Set([...(prev.seenHashes || []), ...(x.seenHashes || []), prev.deviceHash, x.deviceHash].filter(Boolean))]
    });
  });

  const stableKey = cur.deviceStableId || cur.deviceHash;
  const prev = byStable.get(stableKey) || null;
  byStable.set(stableKey, {
    ...prev,
    ...cur,
    firstSeenAt: minPositive(prev?.firstSeenAt, cur.firstSeenAt) || cur.firstSeenAt,
    lastSeenAt: Date.now(),
    seenHashes: [...new Set([...(prev?.seenHashes || []), ...(cur.seenHashes || []), prev?.deviceHash, cur.deviceHash].filter(Boolean))]
  });

  const out = [...byStable.values()]
