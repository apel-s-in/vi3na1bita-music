...
const runSync = async ({ reason = 'autosync', includeSettings = true, pushEnabled = true, maxPullRanges = 50, includeDeviceCatalog = false, legacyCleanup = null } = {}) => {
  ...
  let sharedApplied = { applied: false };
  let templateApplied = false;

  try {
    const sharedDocument = result?.shared?.current || null;
    sharedApplied = sharedDocument ? await applyBackupV7SharedDocument(sharedDocument) : { applied: false };
  ...
  window.dispatchEvent(new CustomEvent('backup:sync:state', { detail: { state: 'ok' } }));
    return { ok: true, authorization, push, pull, rebuild, quarantine: stored.quarantined || [], shared: { applied: sharedApplied.applied === true, changed: result?.shared?.changed === true, hash: safe(result?.shared?.hash) }, legacyCleanup: result?.legacyCleanup || null, settings: result.settings || null };
  } catch (error) {
...
const pullBackupV7Pages = async ({ firstRequest, currentDeviceId, ownerYandexIdHash, maxPages = MAX_PULL_PAGES_PER_SLOT } = {}) => {
  ...
  for (let page = 0; page < Math.max(1, maxPages); page++) {
    ...
    if (Number(result?.pull?.remaining || 0) <= 0) {
      return { pages, result, storedTotal, quarantined, exhausted: false };
    }
  ...
  return { pages, result, storedTotal, quarantined, exhausted: Number(result?.pull?.remaining || 0) > 0 };
};
