...
export const buildBackupDataSnapshot = async () => {
  ...
  return {
    ...
    achievementState: buildAchievementBackupState({
      unlocked: a?.value || {},
      unlockMeta: { ...deriveAchievementUnlockMetaFromEvents(warmTrimmed), ...(aM?.value || {}) },
      profileRpg: uR?.value || { xp: 0, level: 1 },
      streaks: str?.value || {},
      events: warmTrimmed
    }),
...
export const buildBackupRevision = ({ identity: i, devices: dv, data: d, currentDevice: c } = {}) => normalizeCloudBackupMeta({
  ...
  level: Math.max(1, Number(d?.userProfileRpg?.level || 1)),
  xp: Number(d?.userProfileRpg?.xp || 0),
