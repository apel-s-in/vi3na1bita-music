...
export const buildBackupDataSnapshot = async () => {
  ...
  const [st, h, w, a, aM, str, uP, uR, lP, pI, hS, rS, cS, iR] = await Promise.all([metaDB.getAllStats(), metaDB.getEvents('events_hot').catch(()=>[]), metaDB.getEvents('events_warm'), metaDB.getGlobal('unlocked_achievements'), metaDB.getGlobal('achievement_unlock_meta'), metaDB.getGlobal('global_streak'), metaDB.getGlobal('user_profile'), metaDB.getGlobal('user_profile_rpg'), metaDB.getStoreAll('listener_profile').catch(()=>[]), metaDB.getStoreAll('provider_identity').catch(()=>[]), metaDB.getStoreAll('hybrid_sync').catch(()=>[]), metaDB.getStoreAll('recommendation_state').catch(()=>[]), metaDB.getStoreAll('collection_state').catch(()=>[]), metaDB.getStoreAll('intel_runtime').catch(()=>[])]);
  const warmTrimmed = normalizeEventList([...(Array.isArray(w)?w:[]), ...(Array.isArray(h)?h:[])], { limit: 10000 });
  return { stats: st, eventLog: { warm: warmTrimmed }, achievements: a?.value || {}, streaks: str?.value || {}, userProfile: uP?.value || { name: 'Слушатель', avatar: '😎' }, userProfileRpg: uR?.value || { xp: 0, level: 1 }, achievementState: buildAchievementBackupState({ unlocked: a?.value || {}, unlockMeta: { ...deriveAchievementUnlockMetaFromEvents(warmTrimmed), ...(aM?.value || {}) }, profileRpg: uR?.value || { xp: 0, level: 1 }, streaks: str?.value || {}, events: warmTrimmed }), localStorage: collectSharedSnapshotLocalStorage(localStorage), intel: { listenerProfile: dedupIntel(lP), providerIdentity: dedupIntel(pI), hybridSync: dedupIntel(hS), recommendationState: dedupIntel(rS), collectionState: dedupIntel(cS), intelRuntime: dedupIntel(iR) } };
};
