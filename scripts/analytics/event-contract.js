// UID.003_(Event log truth)_(формальный контракт событий для sync/restore/journal)_(event log становится общей правдой, stats остаются rebuildable cache) UID.017_(Launch source stats)_(готовить source-aware события)_(все пользовательские действия получают понятный domain/source) UID.099_(Multi-device sync model)_(merge по eventId)_(события должны быть idempotent и device-aware)
export const EVENT_SCHEMA_VERSION = 1;
export const EVENT_TYPES = Object.freeze({ LISTEN_START: 'LISTEN_START', LISTEN_COMPLETE: 'LISTEN_COMPLETE', LISTEN_SKIP: 'LISTEN_SKIP', FEATURE_USED: 'FEATURE_USED', ACHIEVEMENT_UNLOCK: 'ACHIEVEMENT_UNLOCK', FAVORITE_CHANGED: 'FAVORITE_CHANGED', PLAYLIST_CHANGED: 'PLAYLIST_CHANGED', PROFILE_UPDATED: 'PROFILE_UPDATED', DEVICE_UPDATED: 'DEVICE_UPDATED', AUTH_EVENT: 'AUTH_EVENT', BACKUP_CREATED: 'BACKUP_CREATED', RESTORE_APPLIED: 'RESTORE_APPLIED', SYNC_STATE_CHANGED: 'SYNC_STATE_CHANGED' });
const sS = v => String(v == null ? '' : v).trim(), sN = v => Number.isFinite(Number(v)) ? Number(v) : 0;
export const getEventDomain = t => ({ LISTEN_START: 'listening', LISTEN_COMPLETE: 'listening', LISTEN_SKIP: 'listening', FEATURE_USED: 'feature', ACHIEVEMENT_UNLOCK: 'achievement', FAVORITE_CHANGED: 'favorites', PLAYLIST_CHANGED: 'playlists', PROFILE_UPDATED: 'profile', DEVICE_UPDATED: 'devices', AUTH_EVENT: 'auth', BACKUP_CREATED: 'cloud', RESTORE_APPLIED: 'cloud', SYNC_STATE_CHANGED: 'cloud' }[sS(t)] || 'generic');
...
export const normalizeEventEnvelope = ({ eventId, sessionId, deviceHash, deviceStableId, deviceLabel = '', deviceClass = '', devicePwa = false, deviceOs = '', deviceBrowser = '', deviceLang = '', deviceTimezone = '', deviceScreen = '', platform, type, uid = null, timestamp = Date.now(), data = {} } = {}) => ({ v: EVENT_SCHEMA_VERSION, eventId: sS(eventId) || crypto.randomUUID(), sessionId: sS(sessionId), deviceHash: sS(deviceHash), deviceStableId: sS(deviceStableId), deviceLabel: sS(deviceLabel), deviceClass: sS(deviceClass), devicePwa: !!devicePwa, deviceOs: sS(deviceOs), deviceBrowser: sS(deviceBrowser), deviceLang: sS(deviceLang), deviceTimezone: sS(deviceTimezone), deviceScreen: sS(deviceScreen), platform: sS(platform || 'web'), domain: getEventDomain(type), type: sS(type || 'UNKNOWN'), uid: uid == null ? null : sS(uid), timestamp: sN(timestamp) || Date.now(), data: data && typeof data === 'object' ? data : {} });
