...
class ListeningReceiptService {
  ...
  snapshot(extra = {}) {
    ...
    return { position: Math.max(0, Number(player?.getPosition?.() || 0)), visibility: document.visibilityState || 'visible', platform: platform(), quality: safe(player?.qMode || ''), muted: !!player?.isMuted?.(), volume: Number(player?.getVolume?.() ?? 100), ...extra };
  }
  ...
  async refreshStatus() {
    ...
    this.ingestServerResult(result);
    applyShardRewardResult(result);
    if (result?.wallet || (Array.isArray(result?.grants) && result.grants.length)) {
