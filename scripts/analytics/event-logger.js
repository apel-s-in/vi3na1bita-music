...
class EventLogger {
  ...
  async init() {
    await metaDB.init();
    // Инициализируем стабильный deviceHash
    try {
      const { getOrCreateDeviceHash } = await import('../core/device-identity.js');
      this.deviceHash = await getOrCreateDeviceHash();
    } catch {
      if (!localStorage.getItem('deviceHash')) {
        this.deviceHash = 'dv_' + crypto.randomUUID().replace(/-/g, '').slice(0, 16);
        localStorage.setItem('deviceHash', this.deviceHash);
      } else {
        this.deviceHash = localStorage.getItem('deviceHash');
      }
    }
