...
export const setSleepPreset=async(page,minutes)=>{const modal=page.locator('.modal-bg.active').last(),toggle=modal.locator('#sm-toggle');if(!(await toggle.isChecked()))await toggle.check();await modal.locator('#sm-inp-min').fill(String(minutes));await expect(modal.locator('#sm-btn-apply')).toBeVisible();await modal.locator('#sm-btn-apply').click();};
export const resetSleepTimer=async page=>{const modal=page.locator('.modal-bg.active').last(),toggle=modal.locator('#sm-toggle');if(await toggle.count()){if(await toggle.isChecked())await toggle.uncheck();const close=modal.locator('#sm-btn-close');if(await close.isVisible())await close.click();}else await page.evaluate(()=>window.SleepTimer?.stop?.(false));};
