...
export const waitForPlayback=async page=>page.waitForFunction(()=>window.playerCore?.isPlaying?.()===true,{timeout:10000});
...
export const openSleepTimer=async page=>{await page.click('[data-testid="sleep-open"]');await page.waitForSelector('#sm-inp-min',{timeout:5000});};
export const setSleepPreset=async(page,minutes)=>{await page.fill('#sm-inp-min',String(minutes));await page.click('#sm-btn-apply');};
export const resetSleepTimer=async page=>{const toggle=page.locator('#sm-toggle');if(await toggle.count())await toggle.uncheck();else await page.evaluate(()=>window.SleepTimer?.stop?.(false));};
