...
export const resetSleepTimer=async page=>{const modal=page.locator('.modal-bg.active').last(),toggle=modal.locator('#sm-toggle');if(await toggle.count()){if(await toggle.isChecked()){await modal.locator('.sleep-toggle-switch').click();await expect(toggle).not.toBeChecked();}await page.keyboard.press('Escape');await expect(modal).toBeHidden();}else await page.evaluate(()=>window.SleepTimer?.stop?.(false));};
