...
test('favoritesOnly modal: clicking non-starred track offers choices and cancel keeps previous playback',async({page})=>{await loginByPromo(page);await page.waitForSelector('#track-list .track',{timeout:1e4});const r=page.locator('#track-list .track');await r.nth(0).locator('.like-star').click();await r.nth(0).click();await page.click('#favorites-btn');const b=await page.evaluate(()=>String(window.playerCore?.getCurrentTrack?.()?.uid||''));await r.nth(1).click();await expect(page.locator('.modal-bg.active')).toBeVisible();await page.locator('.modal-bg.active [data-choice="cancel"]').click();await page.waitForTimeout(200);const a=await page.evaluate(()=>String(window.playerCore?.getCurrentTrack?.()?.uid||''));expect(a).toBe(b)});
