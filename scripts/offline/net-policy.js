import { createCloudCorrelationKey, recordYandexStorageResponse } from '../core/cloud-usage-meter.js';
...
const readStats = () => { const m = mk(), def = () => ({ total: 0, monthly: 0, monthKey: m }), o = { wifi: def(), cellular: def(), general: def() }; try { const r = JSON.parse(localStorage.getItem(K.S) || '{}'); for (const k in o) if (r[k]) { o[k].total = Number(r[k].total) || 0; o[k].monthly = r[k].monthKey === m ? (Number(r[k].monthly) || 0) : 0; } } catch {} return o; };
...
const isYandexCloudRequest = request => {
  try {
    return new URL(requestUrl(request), location.href).hostname.endsWith('yandexcloud.net');
  } catch {
    return false;
  }
};
export const trackTrafficFromResponse = (res, forcedType = null, request = null, startedAt = 0, correlationKey = '') => {
  const len = Number(res?.headers?.get?.('content-length') || 0);
  if (len > 0) writeStats(len, getPlatform().supportsNetControl ? (forcedType || detectNetworkType()) : 'general');
...
export const fetchWithTraffic = async (req, init = {}, forcedType = null) => {
  ...
  const correlationKey = isYandexCloudRequest(req) ? createCloudCorrelationKey() : '';
  const headers = new Headers(req instanceof Request ? req.headers : init.headers || {});
  if (correlationKey) headers.set('X-Vi3-Correlation', correlationKey);
  const request = req instanceof Request ? new Request(req, { ...init, headers }) : req;
  const response = await fetch(request, req instanceof Request ? undefined : { ...init, headers });
  return trackTrafficFromResponse(response, forcedType, request, startedAt, correlationKey);
};
