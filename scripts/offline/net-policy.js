...
export const fetchWithTraffic = async (req, init, forcedType = null) => {
  ...
  return trackTrafficFromResponse(res, forcedType);
};
export const guardedFetch = async (req, init, forcedType = null) => {
  if (!isNetworkAllowed()) throw new TypeError('Network blocked by NetPolicy');
  return fetchWithTraffic(req, init, forcedType);
};
...
export const initNetPolicy = () => {
  ...
  window.NetPolicy = { isNetworkAllowed, shouldShowCellularToast, getStatusText, detectNetworkType, getNetPolicyState, guardedFetch, fetchWithTraffic, trackTrafficFromResponse };
