...
export async function resolvePlaybackSource(params) {
  ...
  // ✅ Сеть есть:
  // ТЗ 11.2.A: если OFFLINE mode выключен — Playback Cache работает “исключительно через сеть”.
  // Поэтому при offlineMode=false НЕ используем локальные blob, даже если они есть.
  // Исключение: при отсутствии сети (см. выше) local blob разрешён как единственный вариант.
  if (offlineMode) {
    // По ТЗ 6/7: локальная копия качества ≥ PQ имеет приоритет над сетью (минимизация GitHub).
    if (uid) {
      const want = effectiveQuality;
      ...
      // 1) пробуем желаемое качество (effectiveQuality)
      const localUrl = await tryLocal(want);

      // 2) PQ=Lo: если Lo нет локально, но есть Hi — можем улучшить (ТЗ 6.2)
  ...
  // OFFLINE mode OFF (или OFFLINE ON, но локали нет) — играем по сети в effectiveQuality
