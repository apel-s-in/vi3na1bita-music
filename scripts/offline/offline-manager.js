...
export class OfflineManager {
  ...
  // ТЗ 13: "Обновить все файлы" — тихо, без влияния на playback.
  // v1.0: обновляем только pinned и cloud (transient будет добавлен после расширения мета-слоя).
  async enqueueUpdateAll() {
    const cq = await this.getCacheQuality();

    const pinned = this.getPinnedUids();

    // cloud: те, у кого cloud=true (eligible по TTL) — возьмём через stats
    // В v1.0 нет getAllCloudUids, поэтому пройдём по всем известным трекам в реестре
  ...
  // ТЗ 19.2: детальное состояние трека для UI/алертов/обновлений
  async getTrackOfflineState(uid) {
    ...
    // needsReCache: pinned или cloud-кандидат/auto-cloud, но в CQ нет полной копии
    const needsReCache = (pinned || eligible) && !cqComplete;

    // needsUpdate: если локально полная копия в CQ есть, но remote size для этого качества изменился
    ...
    try {
      ...
      if (meta) {
        ...
        // если оба известны и различаются заметно — нужно обновление
        if (cqComplete && Number.isFinite(curExpected) && curExpected > 0 && Number.isFinite(prevExpected) && prevExpected > 0) {
          ...
          if (diff >= 0.05) needsUpdate = true; // 50KB в MB-единицах (грубая дельта)
        }
      }
    } catch {}
  ...
  async cacheTrackAudio(uid, quality, options = {}) {
    ...
    try {
      ...
      // ТЗ 13: фиксируем "скачано при таком-то expected size" для детекта updates
      try {
