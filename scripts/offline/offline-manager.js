import {
  ...
  setCloudCandidate, getCloudCandidate, clearCloudCandidate, updateGlobalStats, getGlobalStatsAndTotal,
...
class DownloadQueue {
  ...
  getStatus() { 
      return { activeUid: this.active?.uid || null, downloadingKey: this.active?.key || null, queued: this.q.length, isPaused: this.paused }; 
  }
...
export class OfflineManager {
  ...
  async isCloudEligible(uid) {
    ...
    // Исправлено: просто читаем кандидата, не устанавливаем
    const cand = await getCloudCandidate(u);
    if (cand) return true;
  ...
  async _performDownload(uid, quality, kind, userInitiated) {
    ...
    try {
      // Исправлено: использование правильных имен свойств из TrackRegistry
      const src = meta.sources?.audio || {};
      const url = quality === 'lo' ? (src.lo || meta.audio_low || meta.urlLo) : (src.hi || meta.audio || meta.urlHi);
      ...
      if (kind === 'pinned' || kind === 'cloudAuto') await markLocalCloud(uid); 
      else await markLocalTransient(uid, 'window');
    ...
    for (const uid of uids) {
      if (this.isPinned(uid) || (await this.isCloudEligible(uid))) {
        ...
        if (s.needsUpdate) {
          count++;
          const kind = this.isPinned(uid) ? 'pinned' : 'cloudAuto';
          this.enqueueAudioDownload({ uid, quality: cq, priority: PRIORITY.P3_UPDATES, kind, userInitiated: true });
        }
      }
    }
    return { ok: true, count };
