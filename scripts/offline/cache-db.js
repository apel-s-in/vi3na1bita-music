...
export async function updateGlobalStats(uid, deltaSec, deltaFullListens) {
  ...
  const prev = (await promisifyReq(store.get('global'))) || { totalListenSec: 0, totalFullListens: 0 };

  prev.totalListenSec = (Number(prev.totalListenSec) || 0) + (Number(deltaSec) || 0);
  prev.totalFullListens = (Number(prev.totalFullListens) || 0) + (Number(deltaFullListens) || 0);

  return promisifyReq(store.put(prev, 'global'));
}

export async function getGlobalStatsAndTotal() {
  const store = await getStore(STORE_GLOBAL_STATS);
  const g = (await promisifyReq(store.get('global'))) || { totalListenSec: 0, totalFullListens: 0 };
