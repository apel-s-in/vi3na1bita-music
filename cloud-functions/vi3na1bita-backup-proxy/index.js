...
const CFG = {
  ...
  coordinatorRequired: String(process.env.BACKUP_COORDINATOR_REQUIRED || '0').trim() === '1',
  catalogEnabled: String(process.env.BACKUP_CATALOG_ENABLED || '0').trim() === '1',
...
async function authorizeRequest(event, body) {
  ...
  const leaseId = safeId(body?.leaseId, 120);
  const leaseToken = safe(body?.leaseToken);
  const hasLease = !!leaseId && !!leaseToken;

  if (CFG.coordinatorRequired && !hasLease) {
    throw makeError('backup_coordinator_lease_required', 409);
  }

  const authorityAction = hasLease
  ...
  return {
    ...
    initializationRequired: authorization.initializationRequired === true,
    initializationMode: safe(authorization.initializationMode),
    settingsSourceDeviceId: safeId(authorization.settingsSourceDeviceId, 120)
  };
}
...
async function ensureDir(token, path) {
  ...
  if (usage?.ensuredDirs?.has(cleanPath)) return true;
  bumpDiskOperation('mkdir');
  const created = await diskJson('PUT', `${API}/resources?path=${encodeURIComponent(cleanPath)}`, token, null, 10000);
  if (![200, 201, 202, 409].includes(created.status)) {
    const status = [401, 403, 507].includes(created.status) ? created.status : 502;
    throw makeError(created.status === 507 ? 'disk_space_exhausted' : `disk_directory_create_failed:${created.status}`, status);
  }
  usage?.ensuredDirs?.add(cleanPath);
  return true;
}
...
async function uploadJson(token, path, value, { overwrite = false } = {}) {
  ...
  if (link.status !== 200 || !link.json?.href) {
    const status = [401, 403, 409, 507].includes(link.status) ? link.status : 502;
    throw makeError(link.status === 507 ? 'disk_space_exhausted' : `disk_upload_link_failed:${link.status}`, status);
  }
  ...
  if (written.status < 200 || written.status >= 300) {
    const status = [401, 403, 409, 507].includes(written.status) ? written.status : 502;
    throw makeError(written.status === 507 ? 'disk_space_exhausted' : `disk_upload_failed:${written.status}`, status);
  }
...
function normalizeRange(raw, auth) {
  ...
  if (version !== VERSION) throw makeError('range_version_invalid', 400);
  ...
  const core = { version: VERSION, ownerYandexIdHash: auth.ownerYandexIdHash, deviceId, chainId, fromSeq, toSeq, previousRangeHash, events, createdAt };
...
function verifyStoredRange(raw, expected = {}) {
  ...
  if (safe(raw.version) !== VERSION) throw makeError('stored_range_version_invalid', 502);
  const core = {
    version: VERSION,
...
async function writeImmutableRange(auth, rawRange) {
  ...
  if (!head) {
    ...
    if (safe(range.previousRangeHash) !== safe(head.lastRangeHash)) {
...
function normalizeCatalogHead(raw = {}) {
  const deviceId = safeId(raw.deviceId, 120);
  const chainId = safeId(raw.chainId, 160);
  const lastToSeq = integer(raw.lastToSeq);
  const lastRangeHash = safe(raw.lastRangeHash);
  if (!deviceId || !chainId || lastToSeq < 1 || !isHash(lastRangeHash)) return null;
  return {
    deviceId,
    chainId,
    lastFromSeq: integer(raw.lastFromSeq),
    lastToSeq,
    lastRangeHash,
    lastEventHash: safe(raw.lastEventHash),
    ranges: integer(raw.ranges),
    events: integer(raw.events),
    updatedAt: integer(raw.updatedAt)
  };
}

function catalogCore(raw = {}, ownerYandexIdHash = '') {
  const chains = [...new Map(
    (Array.isArray(raw.chains) ? raw.chains : [])
      .map(normalizeCatalogHead)
      .filter(Boolean)
      .map(head => [`${head.deviceId}:${head.chainId}`, head])
  ).values()]
    .sort((left, right) => left.deviceId.localeCompare(right.deviceId) || left.chainId.localeCompare(right.chainId))
    .slice(0, MAX_PULL_CHAINS);
...
function buildCatalogDocument(auth, chains = [], revision = 0) {
  const core = catalogCore({
    revision,
    chains,
    updatedAt: now()
  }, auth.ownerYandexIdHash);
  return { ...core, hash: hash(stableStringify(core)) };
}

function verifyCatalogDocument(raw, auth) {
  if (!isObject(raw)) throw makeError('catalog_document_invalid', 502);
  const core = catalogCore(raw, raw.ownerYandexIdHash);
  if (core.version !== VERSION || core.schemaVersion !== CATALOG_SCHEMA_VERSION) {
    throw makeError('catalog_version_invalid', 502);
  }
  if (core.ownerYandexIdHash !== auth.ownerYandexIdHash) {
    throw makeError('catalog_owner_mismatch', 403);
  }
  const expectedHash = hash(stableStringify(core));
  if (!isHash(raw.hash) || safe(raw.hash) !== expectedHash) {
    throw makeError('catalog_hash_mismatch', 502);
  }
  return { ...core, hash: expectedHash };
}

async function writeCatalogDocument(auth, document) {
  await ensureDirs(auth.oauthToken, [BACKUP_ROOT, ROOT]);
  await uploadJson(auth.oauthToken, CATALOG_PATH, document, { overwrite: true });
  return document;
}

async function rebuildCatalogDocument(auth, reason = 'catalog_bootstrap') {
  const heads = await listChainHeads(auth);
  const document = buildCatalogDocument(auth, heads, 1);
  await writeCatalogDocument(auth, document);
  return { document, rebuilt: true, reason };
}

async function readOrRebuildCatalog(auth) {
  const raw = await downloadJson(auth.oauthToken, CATALOG_PATH, { maxBytes: 1024 * 1024 });
  if (!raw) return rebuildCatalogDocument(auth, 'catalog_missing');

  try {
    return {
      document: verifyCatalogDocument(raw, auth),
      rebuilt: false,
      reason: ''
    };
  } catch (error) {
    if (Number(error?.status || 0) === 403 || /owner_mismatch/.test(safe(error?.message))) {
      throw error;
    }
    return rebuildCatalogDocument(auth, 'catalog_invalid');
  }
}

function updateCatalogHead(documentRaw, result, auth) {
  const current = verifyCatalogDocument(documentRaw, auth);
  const incoming = normalizeCatalogHead({
    deviceId: result.range.deviceId,
    chainId: result.range.chainId,
    lastFromSeq: result.head?.lastFromSeq || result.range.fromSeq,
    lastToSeq: result.head?.lastToSeq || result.range.toSeq,
    lastRangeHash: result.head?.lastRangeHash || result.range.hash,
    lastEventHash: result.head?.lastEventHash || result.range.events?.[result.range.events.length - 1]?.eventHash,
    ranges: result.head?.ranges || 1,
    events: result.head?.events || result.range.events?.length,
    updatedAt: result.head?.updatedAt || now()
  });
  if (!incoming) throw makeError('catalog_head_invalid', 502);

  const key = `${incoming.deviceId}:${incoming.chainId}`;
  const heads = new Map(current.chains.map(head => [`${head.deviceId}:${head.chainId}`, head]));
  const old = heads.get(key);

  if (
    old &&
    old.lastToSeq === incoming.lastToSeq &&
    old.lastRangeHash === incoming.lastRangeHash &&
    old.lastEventHash === incoming.lastEventHash
  ) {
    return { changed: false, document: current };
  }

  heads.set(key, incoming);
  return {
    changed: true,
    document: buildCatalogDocument(auth, [...heads.values()], current.revision + 1)
  };
}
async function pullAfterWatermarks(auth, rawWatermarks, { maxRanges = MAX_PULL_RANGES, maxBytes = MAX_RESPONSE_BYTES - RESPONSE_RESERVE_BYTES, catalogHeads = null } = {}) {
  const watermarks = normalizeWatermarks(rawWatermarks);
  const heads = (Array.isArray(catalogHeads) ? catalogHeads : await listChainHeads(auth))
    .map(normalizeCatalogHead)
    .filter(Boolean)
    .sort((left, right) => left.deviceId.localeCompare(right.deviceId) || left.chainId.localeCompare(right.chainId));
  const ranges = [];
  ...
  for (const head of heads) {
    ...
    while (cursor < head.lastToSeq) {
      ...
      if (safe(verified.previousRangeHash) !== lastRangeHash) {
...
function normalizeSharedPlaylist(raw = {}) {
  ...
  return {
    id,
    name: safe(raw.name || 'Плейлист').slice(0, 120) || 'Плейлист',
    color: safe(raw.color).slice(0, 40),
    order: uniqueIds(raw.order, MAX_SHARED_PLAYLIST_TRACKS),
    hidden: uniqueIds(raw.hidden, MAX_SHARED_PLAYLIST_TRACKS),
    ops: (Array.isArray(raw.ops) ? raw.ops : [])
      .map(operation => {
        const type = ['add', 'del'].includes(safe(operation?.t)) ? safe(operation.t) : '';
        const uid = safeId(operation?.u, 160);
        return type && uid ? { t: type, u: uid, ts: integer(operation?.ts) } : null;
      })
      .filter(Boolean)
      .slice(-MAX_SHARED_PLAYLIST_OPS),
    createdAt: integer(raw.createdAt),
    updatedAt: integer(raw.updatedAt),
...
function pickSharedRow(left, right, clock) {
  ...
  if (rightClock > leftClock) return right;
...
async function exchangeSharedDocument(auth, rawShared, knownHash = '', { readEnabled = true, writeEnabled = true } = {}) {
  if (!readEnabled && !writeEnabled) {
    return { skipped: true, changed: false, migrated: false, pushed: false, current: null, hash: safe(knownHash) };
  }
  ...
  if (!writeEnabled || !isObject(rawShared)) {
    return {
      skipped: false,
      changed: false,
      migrated: false,
      pushed: false,
      current: readEnabled && current && current.hash !== safe(knownHash) ? publicSharedDocument(current) : null,
      hash: current?.hash || ''
    };
  }
  ...
  return {
    ...
    pushed: true,
    current: readEnabled ? publicSharedDocument(merged.document) : null,
...
async function putSettings(auth, rawSettings) {
  ...
  const existing = await downloadJson(auth.oauthToken, path, { maxBytes: 2 * 1024 * 1024 });
...
function publicAuthorization(auth) {
  return {
    ...
    initializationRequired: auth.initializationRequired,
    initializationMode: auth.initializationMode,
    settingsSourceDeviceId: auth.settingsSourceDeviceId,
    device: auth.device
  };
}
async function runBatchSync(auth, body) {
  if (auth.initializationRequired) {
    throw makeError('backup_device_initialization_required', 409);
  }
  if (CFG.catalogEnabled && !CFG.coordinatorRequired) {
    throw makeError('backup_catalog_requires_mandatory_coordinator', 503);
  }
  ...
  if (CFG.catalogEnabled && (pushRanges.length > 0 || body.includePull !== false)) {
    catalogState = await readOrRebuildCatalog(auth);
    catalog = catalogState.document;
  }
  ...
  for (const rawRange of sortedPushRanges) {
    ...
    if (catalog) {
      const updated = updateCatalogHead(catalog, result, auth);
      catalog = updated.document;
      catalogChanged = catalogChanged || updated.changed;
    }
  }

  if (catalog && catalogChanged) {
    await writeCatalogDocument(auth, catalog);
  }
  ...
  const legacySharedEnabled = body.includeShared !== false;
  const sharedReadEnabled = body.includeSharedRead == null ? legacySharedEnabled : body.includeSharedRead === true;
  const sharedWriteEnabled = body.includeSharedWrite == null ? legacySharedEnabled && isObject(body.shared) : body.includeSharedWrite === true;
  const shared = await exchangeSharedDocument(auth, body.shared, body.knownSharedHash, {
    readEnabled: sharedReadEnabled,
    writeEnabled: sharedWriteEnabled
  });
  ...
  const pull = body.includePull === false
    ...
    : await pullAfterWatermarks(auth, body.watermarks || body.knownWatermarks || [], {
        ...
        catalogHeads: catalog?.chains || null
      });
  ...
  return {
    ...
    catalog: catalog ? {
      ...
      rebuilt: catalogState?.rebuilt === true,
      rebuildReason: safe(catalogState?.reason),
    ...
    if (mode === 'ping') {
      return reply(
        ...
        200,
        {
          ...
          backupCoordinator: {
            supported: true,
            required: CFG.coordinatorRequired,
            authorityAction: 'backup_sync_authorize'
          },
