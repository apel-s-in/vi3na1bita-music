  ...
  const items = [...new Map(
    source
      .map(item => {
        ...
        const id = `backup_${sha256Hex(
          `${itemOwner}:${payloadHash}`
        ).slice(0, 40)}`;
        ...
        return [
          ...
          {
            id,
            ownerYandexId: itemOwner,
            payloadHash,
            queuedAt,
            attempts: Math.max(0, Math.floor(safeNum(item?.attempts))),
            lastAttemptAt: Math.max(0, safeNum(item?.lastAttemptAt)),
            lastError: safeString(item?.lastError).slice(0, 200)
          }
        ];
      })
      .filter(Boolean)
...
async function readBackupReceiptOutbox(token, ownerYandexId) {
  const result = await downloadJsonResourceByPath(
    token,
    BACKUP_RECEIPT_OUTBOX_PATH,
    {
      link: 'backup_receipt_outbox_link',
      parse: 'backup_receipt_outbox_link_parse',
      file: 'backup_receipt_outbox_file',
      json: 'backup_receipt_outbox_json'
    }
  );
  ...
  if (result.type === 'not_found') {
    return {
      exists: false,
      outbox: normalizeBackupReceiptOutbox(
        {},
        ownerYandexId
      )
    };
  ...
  if (result.type !== 'ok') {
    throw new Error(
      `backup_receipt_outbox_read_failed:${result.type}`
    );
  }
...
async function writeBackupReceiptOutbox(token, outbox) {
  ...
  const result = await uploadJsonResourceByPath(
    token,
    BACKUP_RECEIPT_OUTBOX_PATH,
    outbox
  );
...
async function flushBackupAchievementReceipts(token, validation) {
  ...
  try {
    stored = await readBackupReceiptOutbox(
      token,
      validation.ownerYandexId
    );
  } catch (error) {
    const direct = await emitBackupAchievementReceipt(
      validation
    ).catch(sendError => ({
      ok: false,
      skipped: false,
      error: normalizeErrMessage(sendError)
    }));

    return {
      ...direct,
      durable: false,
      outboxError: normalizeErrMessage(error)
    };
  }

  const currentId = `backup_${sha256Hex(
    `${validation.ownerYandexId}:${validation.payloadHash}`
  ).slice(0, 40)}`;

  const rows = new Map(
    stored.outbox.items.map(item => [item.id, item])
  );
  ...
  const nextOutbox = normalizeBackupReceiptOutbox(
    { items: pending },
    validation.ownerYandexId
  );

  if (stored.exists || nextOutbox.items.length) {
    await writeBackupReceiptOutbox(
      token,
      nextOutbox
    );
  }
  ...
  return {
    ok:
      nextOutbox.items.length === 0 &&
      currentResult?.ok === true,
    durable: true,
    delivered,
    pending: nextOutbox.items.length,
  ...
  if (mode.startsWith('upload_')) {
    try {
      ...
      const rewardReceipt =
        ...
        path === BACKUP_PATH &&
        backupValidation?.ok
          ? await flushBackupAchievementReceipts(
              token,
              backupValidation
            ).catch(error => ({
              ok: false,
