...
const publicCoordinatorState = (raw, currentDeviceId = '', at = Date.now()) => {
  ...
  return {
    ...
    retryAt: state.lease?.expiresAt || (queueIndex >= 0 ? at + (state.queue[queueIndex].manual ? MANUAL_RETRY_MS : DEFAULT_RETRY_MS) : 0),
...
const claimCoordinatorLease = (raw, input = {}, options = {}) => {
  ...
  if (state.lease?.holderDeviceId === deviceId) {
    const existingTokenHash = safe(input.existingTokenHash).slice(0, 64);
    const tokenMatches = !!existingTokenHash && existingTokenHash === state.lease.tokenHash;
    return {
      changed: changedState(before, state),
      state,
      result: tokenMatches
  ...
  return {
    ...
    result: {
      ...
      retryAt: at + (ticket.manual ? MANUAL_RETRY_MS : DEFAULT_RETRY_MS)
    }
  };
};
