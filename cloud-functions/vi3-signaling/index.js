...
async function kvCompareAndPut({ row, type = '', owner = '', expiresAt = 0, data = {} }) {
  ...
  if (!expectedPayloadJson) throw new Error('cas_payload_required');
  ...
  `,
    {
      ...
      $expires_at_ms: tvUint64(expiresAt),
      $payload_json: tvUtf8(JSON.stringify(data || {}))
    }
  );

  const changed = rowsOf(result).some(item => safe(item.pk) === safe(row.pk));
...
async function enforceRateLimit({ scope, actor, limit, windowMs }) {
  ...
  if (!cleanScope || !cleanActor) throw new Error('rate_limit_identity_required');
  ...
  const resetAt = (bucket + 1) * duration;
  const actorHash = hash(`rate:${cleanActor}`).slice(0, 24);
  const pk = `rateBucket:${cleanScope}:${actorHash}:${bucket}`;
  const expiresAt = resetAt + 60000;

  for (let attempt = 0; attempt < 8; attempt++) {
    const row = await kvGet(pk);
    const current = Math.max(0, Math.floor(num(payload(row)?.count)));
    if (current >= max) {
      const error = new Error('rate_limit_exceeded');
      error.httpStatus = 429;
      error.retryAfterMs = Math.max(1000, resetAt - at);
      throw error;
    }

    const data = { scope: cleanScope, actorHash, bucket, count: current + 1, resetAt, updatedAt: at };
    if (!row) {
      try {
        await kvInsert({ pk, type: 'rateLimitBucket', owner: actorHash, expiresAt, data });
    ...
    if (await kvCompareAndPut({ row, type: 'rateLimitBucket', owner: actorHash, expiresAt, data })) {
      return { ok: true, remaining: Math.max(0, max - data.count), resetAt };
    }
  }

  const error = new Error('rate_limit_conflict');
  error.httpStatus = 429;
  throw error;
}
...
function publicAccountDevice(raw = {}) {
  ...
  return {
    ...
    archived: device.lastSeenAt > 0 && device.lastSeenAt < now() - ACCOUNT_DEVICE_ARCHIVE_MS,
...
async function actionAccountDeviceList(event, body) {
  const auth = await requirePlayer(event, body);
  const rows = await kvPrefix(`accountDevice:${auth.playerId}:`, 100);
  const all = rows
    .map(payload)
    .map(item => normalizeAccountDevice(item, auth.playerId))
    .filter(item => item.deviceId)
    .sort((left, right) => right.lastSeenAt - left.lastSeenAt);
  const archived = all.filter(item => item.deviceId !== auth.deviceId && item.lastSeenAt > 0 && item.lastSeenAt < now() - ACCOUNT_DEVICE_ARCHIVE_MS);
  const visible = body.includeArchived === true ? all : all.filter(item => !archived.includes(item));
  return {
    ok: true,
    items: visible.map(publicAccountDevice),
    archivedCount: archived.length,
    archiveAfterMs: ACCOUNT_DEVICE_ARCHIVE_MS
  };
}
...
async function actionAccountDeviceUpdate(event, body) {
  ...
  const revokedResources = body.revoked === true
    ? {
        webPushSubscriptions: await removeDeviceWebPushSubscriptions(playerId, deviceId),
        presence: await kvDelete(`presence:${playerId}:${deviceId}`).then(() => 1).catch(() => 0),
        listenActive: await kvDelete(`listenActive:${playerId}:${deviceId}`).then(() => 1).catch(() => 0),
        sleepTimerActive: await kvDelete(`sleepTimerActive:${playerId}:${deviceId}`).then(() => 1).catch(() => 0)
      }
    : null;
  return { ok: true, device: publicAccountDevice(next), revokedResources };
}
...
async function actionBackupDeviceAuthorize(event, body) {
  const auth = await requirePlayer(event, body);
  const requestedDeviceId = sanitizeId(body.deviceId, 120);

  if (!auth.deviceId) {
    throw backupAuthorizationError('backup_device_session_required', 401);
  }
  if (requestedDeviceId && requestedDeviceId !== auth.deviceId) {
    throw backupAuthorizationError('backup_device_identity_mismatch');
  }
  if (!/^[a-f0-9]{64}$/.test(auth.yandexIdHash)) {
    throw backupAuthorizationError('backup_owner_hash_invalid', 401);
  }

  const [deviceRow, playerRow] = await Promise.all([
    kvGet(accountDeviceKey(auth.playerId, auth.deviceId)),
    kvGet(`player:${auth.playerId}`)
  ]);
  const device = normalizeAccountDevice(payload(deviceRow), auth.playerId);
  const player = payload(playerRow);

  if (!deviceRow || !device.deviceId || device.deviceId !== auth.deviceId) {
    throw backupAuthorizationError('backup_device_not_registered');
  }
  if (device.revokedAt > 0) {
    throw backupAuthorizationError('backup_device_revoked');
  }
  if (!playerRow || safe(player.yandexIdHash) !== auth.yandexIdHash) {
    throw backupAuthorizationError('backup_owner_identity_mismatch');
  }
...
async function closeActiveListenSegment(playerId, deviceId, position, reason = 'ownership_transfer') {
  ...
  for (let attempt = 0; attempt < 8; attempt++) {
    ...
    if (!(await kvCompareAndPut({ row, type: 'listenActive', owner: playerId, expiresAt: at + LISTEN_ACTIVE_RETENTION_MS, data: completed }))) continue;
...
async function getPlaybackCoordinationState(playerId, currentDeviceId = '') {
  ...
  return {
    known: true,
    required: remoteOwnerActive || peers.length > 0,
    reason: remoteOwnerActive ? 'remote_playback_owner' : peers.length ? 'recent_peer_activity' : 'single_device',
...
async function actionPlaybackStateGet(event, body) {
  ...
  return {
    ok: true,
    playback: publicPlaybackState(state, auth.deviceId || body.deviceId),
    coordination: await getPlaybackCoordinationState(auth.playerId, auth.deviceId || body.deviceId)
  };
}
...
async function actionSocialSessionIssue(event, body) {
  ...
  const playbackCoordination = await getPlaybackCoordinationState(identity.friendId, accountDevice?.deviceId || body.deviceId).catch(() => ({
    known: false,
    required: false,
    reason: 'coordination_check_unavailable',
    activePeerCount: 0,
    activePeerDeviceIds: [],
  ...
  return {
    ...
    ownerYandexIdHash: yandexIdHash,
...
async function requirePlayer(event, body) {
  ...
  return {
    ...
    deviceId: sanitizeId(claims.did, 120),
    yandexIdHash: safe(claims.yidHash),
    expiresAt: num(claims.exp),
    authVersion: Math.max(2, Math.floor(num(claims.v, 2)))
  };
}
...
async function actionFriendInviteAccept(event, body) {
  ...
  const webPush = await sendSystemWebPush({
    toPlayerId: inv.fromPlayerId,
    title: '👥 Новый друг',
    body: `${myProfile.displayName || 'Пользователь'} принял приглашение`,
    url: './?openFriends=1',
    tag: `friends-${playerId}`,
    requireInteraction: false,
    kind: 'FRIENDS_CHANGED',
    fromFriendId: playerId
  }).catch(error => ({ ok: false, error: safe(error?.message) }));

  return { ok: true, friendPlayerId: inv.fromPlayerId, webPush };
}
...
async function readFriendItems(playerId) {
  const rows = await kvPrefix(`friend:${sanitizeId(playerId)}:`, 300);
  const items = [];

  for (const row of rows) {
    const data = payload(row);
    if (!data || (data.status && data.status !== 'active')) continue;
    ...
    if (!profile?.displayName) {
      const stored = payload(await kvGet(`profile:${friendId}`));
      if (stored?.friendId) profile = stored;
    }
  ...
  return items.sort((left, right) => num(right.createdAt) - num(left.createdAt));
}

async function actionFriendList(event, body) {
  const { playerId } = await requirePlayer(event, body);
  return { ok: true, items: await readFriendItems(playerId) };
}
async function actionFriendRemove(event, body) {
  ...
  const webPush = await sendSystemWebPush({
    toPlayerId: targetId,
    title: '👥 Список друзей изменён',
    body: 'Обновите раздел «Друзья»',
    url: './?openFriends=1',
    tag: `friends-${playerId}`,
    requireInteraction: false,
    kind: 'FRIENDS_CHANGED',
  ...
  return { ok: true, removed: true, webPush };
}
async function readPresenceForFriendIds(friendIds = []) {
  const ids = [...new Set((Array.isArray(friendIds) ? friendIds : []).map(id => sanitizeId(id)).filter(Boolean))].slice(0, 50);
  const presence = {};
  const at = now();

  for (const id of ids) {
    const rows = await kvPrefix(`presence:${id}:`, 10);
    const best = rows.map(payload).sort((left, right) => num(right.onlineUntil) - num(left.onlineUntil))[0] || null;
    presence[id] = {
      online: !!best && num(best.onlineUntil) > at,
      lastSeenAt: Math.max(0, num(best?.updatedAt)),
      currentGameId: sanitizeId(best?.currentGameId, 80),
      currentRoomId: sanitizeId(best?.currentRoomId, 96)
    };
  }
...
async function actionPresenceBatch(event, body) {
  ...
  return { ok: true, presence: await readPresenceForFriendIds(ids) };
}

async function actionFriendsSnapshot(event, body) {
  const { playerId } = await requirePlayer(event, body);
  const items = await readFriendItems(playerId);
  const presence = await readPresenceForFriendIds(items.map(item => item.friendId));
  const generatedAt = now();
  const revision = hash(stableStringify({
...
const LISTEN_SHORT_COMPLETION_MS = LISTEN_VALID_MIN_SEC * 1000;
const LOGICAL_START_TOLERANCE_MS = 6000;
const LISTEN_SESSION_RETENTION_MS = 90 * 24 * 60 * 60 * 1000;
const LISTEN_RECEIPT_RETENTION_MS = 180 * 24 * 60 * 60 * 1000;
const LISTEN_ACTIVE_RETENTION_MS = CFG.listenSessionMaxMs + CFG.listenBackgroundMaxGapMs + 60 * 60 * 1000;
...
async function finalizeLogicalListen(logicalRaw) {
  ...
  const full = logical.reachedEndNaturally && !logical.invalidated && logical.initialStartedPositionMs <= LOGICAL_START_TOLERANCE_MS && logical.finalPositionMs >= Math.max(0, logical.durationMs - 3000) && logical.coveredMs >= requiredCoverageMs && logical.acceptedCreditMs >= requiredCoverageMs && logical.maxGapMs <= LOGICAL_COVERAGE_MAX_GAP_MS;
  const receipt = {
    version: LISTEN_RECEIPT_VERSION,
    receiptId,
  ...
  if (!oldRow) {
    try {
      await kvInsert({ pk: receiptPk, type: 'listenReceipt', owner: logical.playerId, expiresAt: Math.max(now(), logical.completedAt) + LISTEN_RECEIPT_RETENTION_MS, data: { ...receipt, progressApplied: false } });
    } catch {}
  }
  ...
  await kvPut({ pk: receiptPk, type: 'listenReceipt', owner: logical.playerId, expiresAt: Math.max(now(), logical.completedAt) + LISTEN_RECEIPT_RETENTION_MS, data: completedReceipt });
  for (let attempt = 0; attempt < 8; attempt++) {
...
function calculateServerStreakSummary(days = []) {
  ...
  const lastActiveDate = values[values.length - 1] || '';
  const current = lastActiveDate === today || lastActiveDate === yesterday ? run : 0;

  return { current, longest, lastActiveDate, activeDays: values.length };
}
...
function publicAchievementProgressCompact(progress) {
  const result = publicAchievementProgress(progress);
  delete result.confirmedListeningStats;
  return result;
}
...
async function finalizeListenSession(session) {
  const data = normalizeListenSession(session);
  if (!data.logicalSessionId) {
    const error = new Error('logical_listen_session_required');
    error.httpStatus = 409;
    throw error;
  }

  const logical = await syncLogicalListenSession(data);
  const appliedTime = await applyVerifiedListenTimeProgress(data);
  const observedSec = Math.max(0, Math.floor(data.observedMs / 1000));
  const valid = data.observedMs >= LISTEN_SHORT_COMPLETION_MS;
  const segmentReceipt = {
    version: LISTEN_RECEIPT_VERSION,
    receiptKind: 'device_segment',
    persisted: false,
  ...
  const shouldFinalizeLogical = logical?.status === 'completed' && (data.completionReason === 'ended' || valid);
  const logicalFinalized = shouldFinalizeLogical ? await finalizeLogicalListen(logical) : null;
  const progress = logicalFinalized?.progress || appliedTime.progress;
  const previousTotalSec = Math.floor(appliedTime.previousTotalListenMs / 1000);
  const crossedTimeReward = buildTimeRewards(progress.totalSec).some(reward => reward.target > previousTotalSec && reward.target <= progress.totalSec);
  const rewards = logicalFinalized?.rewards || (crossedTimeReward
    ? await reconcileAchievementRewards(data.playerId, progress, { metrics: ['totalSec'] })
    : { enabled: true, grants: [], wallet: null });

  return {
    receipt: logicalFinalized?.receipt || segmentReceipt,
    segmentReceipt,
    logical: logical ? publicLogicalListen(logical) : null,
    progress,
    rewards,
    duplicate: logicalFinalized ? logicalFinalized.duplicate : appliedTime.duplicate
  };
}
async function actionListenSessionStart(event, body) {
  ...
  if (!fenced.state.logicalSessionId) {
    const error = new Error('logical_listen_session_required');
    error.httpStatus = 409;
    throw error;
  }
  ...
  for (let attempt = 0; attempt < 10; attempt++) {
    ...
    if (!row) {
      try {
        await kvInsert({ pk: key, type: 'listenActive', owner: playerId, expiresAt: at + LISTEN_ACTIVE_RETENTION_MS, data: session });
    ...
    if (!(await kvCompareAndPut({ row, type: 'listenActive', owner: playerId, expiresAt: at + LISTEN_ACTIVE_RETENTION_MS, data: session }))) {
      continue;
    }
    await persistListenSession(session);
    const logical = await syncLogicalListenSession(session);
...
async function actionListenSessionHeartbeat(event, body) {
  ...
  for (let attempt = 0; attempt < 10; attempt++) {
    ...
    if (!(await kvCompareAndPut({ row, type: 'listenActive', owner: playerId, expiresAt: now() + LISTEN_ACTIVE_RETENTION_MS, data: observation.session }))) {
      continue;
    }
    ...
    return {
      ...
      loyalty: loyalty?.loyalty || null,
      wallet: loyalty?.wallet ? publicShardWallet(loyalty.wallet) : rewards.wallet ? publicShardWallet(rewards.wallet) : null,
      progress: publicAchievementProgressCompact(appliedTime.progress),
...
async function actionListenSessionComplete(event, body) {
  ...
  for (let attempt = 0; attempt < 10; attempt++) {
    ...
    if (current.status === 'completed') {
      ...
      const loyalty = finalized.receipt.valid === true ? await applyLoyaltyActivity(playerId, { activityId: `loyalty:listen:${sessionId}`, kind: 'listening', deviceId: current.deviceId }) : null;
      return {
        ok: true,
        duplicate: true,
        shadow: CFG.listeningReceiptsShadow,
    ...
    const changed = await kvCompareAndPut({
      row,
      type: useActive ? 'listenActive' : 'listenSession',
      owner: playerId,
      expiresAt: useActive ? at + LISTEN_ACTIVE_RETENTION_MS : num(row.expires_at) || at + LISTEN_SESSION_RETENTION_MS,
      data: completed
    });
    if (!changed) continue;
    if (useActive) {
    ...
    const finalized = await finalizeListenSession(completed);
    if (useActive) await kvDelete(listenActiveKey(playerId, completed.deviceId)).catch(() => null);
    const loyalty = finalized.receipt.valid === true ? await applyLoyaltyActivity(playerId, { activityId: `loyalty:listen:${sessionId}`, kind: 'listening', deviceId: completed.deviceId }) : null;
    return {
      ok: true,
      duplicate: false,
...
function webPushEndpointKey(endpointHash) {
  return `webPushEndpoint:${sanitizeId(endpointHash, 64)}`;
}

async function deleteWebPushSubscriptionRow(row) {
  if (!row?.pk || !row?.payload_json) return false;
  const data = payload(row);
  const endpointHash = sanitizeId(data.endpointHash, 64);
  ...
  if (remaining || !endpointHash) return !remaining;

  const indexKey = webPushEndpointKey(endpointHash);
...
async function removeDeviceWebPushSubscriptions(playerId, deviceId, exceptEndpointHash = '') {
  const cleanPlayerId = sanitizeId(playerId, 96);
  const cleanDeviceId = sanitizeId(deviceId, 120);
  const except = sanitizeId(exceptEndpointHash, 64);
  if (!cleanPlayerId || !cleanDeviceId) return 0;
  const rows = await kvPrefix(`webPushSub:${cleanPlayerId}:`, 100);
  const targets = rows.filter(row => {
    const data = payload(row);
    return sanitizeId(data.deviceId, 120) === cleanDeviceId && sanitizeId(data.endpointHash, 64) !== except;
  });
  let removed = 0;
  for (const row of targets) {
    if (await deleteWebPushSubscriptionRow(row)) removed++;
  }
  return removed;
}
...
async function actionWebPushSubscribe(event, body) {
  ...
  const subscriptionKey = `webPushSub:${auth.playerId}:${endpointHash}`;
  const endpointKey = webPushEndpointKey(endpointHash);
  const [oldRow, endpointRow] = await Promise.all([kvGet(subscriptionKey), kvGet(endpointKey)]);
  const old = payload(oldRow);
  const endpointOwner = payload(endpointRow);
  const at = now();
  const leaseExpiresAt = at + WEB_PUSH_LEASE_MS;
  ...
  const previousSubscriptionKey = safe(endpointOwner.subscriptionKey);
  if (previousSubscriptionKey && previousSubscriptionKey !== subscriptionKey) {
    const previousRow = await kvGet(previousSubscriptionKey).catch(() => null);
    if (previousRow) await deleteWebPushSubscriptionRow(previousRow).catch(() => false);
  }

  const replacedForDevice = await removeDeviceWebPushSubscriptions(auth.playerId, auth.deviceId, endpointHash);
  ...
  return {
    ...
    leaseExpiresAt,
    replacedForDevice,
    transferredEndpoint: !!previousSubscriptionKey && previousSubscriptionKey !== subscriptionKey
  };
}
async function actionWebPushUnsubscribe(event, body) {
  ...
  const endpointHash = hash(endpoint).slice(0, 32);
  const row = await kvGet(`webPushSub:${auth.playerId}:${endpointHash}`);
  if (!row) return { ok: true, removed: false };
  const removed = await deleteWebPushSubscriptionRow(row);
...
const ACTIONS = {
  ...
  friends_snapshot: actionFriendsSnapshot,
    ...
    if (action === 'ping') {
      return reply(event, 200, {
        ...
        listeningReceipts: {
          ...
          activeRetentionMs: LISTEN_ACTIVE_RETENTION_MS,
          persistentSegmentReceipts: false,
        ...
        backupCoordinator: {
          ...
          activePeerOnlyQueue: true,
          automaticRetryMs: 60000,
          manualRetryMs: 15000
        },
