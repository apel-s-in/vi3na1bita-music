...
async function deleteWebPushSubscriptionRow(row) {
  ...
  await kvDelete(row.pk).catch(() => null);
  if (!endpointHash) return true;
  const indexKey = webPushEndpointKey(endpointHash);
  const indexRow = await kvGet(indexKey);
  const index = payload(indexRow);
  if (indexRow && safe(index.subscriptionKey) === safe(row.pk)) {
    await kvDelete(indexKey).catch(() => null);
  }
  return true;
}
...
async function removeDeviceWebPushSubscriptions(playerId, deviceId, exceptEndpointHash = '') {
  ...
  for (const row of targets) await deleteWebPushSubscriptionRow(row);
...
async function actionWebPushSubscribe(event, body) {
  ...
  if (previousSubscriptionKey && previousSubscriptionKey !== subscriptionKey) {
    await kvDelete(previousSubscriptionKey).catch(() => null);
  }
