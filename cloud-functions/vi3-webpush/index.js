...
const safeId = (value, max = 120) => safe(value).replace(/[^A-Za-z0-9._:-]/g, '').slice(0, max);
...
async function deleteSubscriptionRow(row) {
  ...
  const endpointHash = safe(data.endpointHash).replace(/[^A-Za-z0-9._:-]/g, '').slice(0, 64);
...
function subscriptionLeaseExpiresAt(row) {
  const data = payload(row);
  return Math.max(0, num(data.leaseExpiresAt) || num(data.updatedAt) + WEB_PUSH_LEASE_MS);
}
...
async function sendToSubscription(row, notification) {
  ...
  try {
    ...
    if (status === 404 || status === 410) {
      await deleteSubscriptionRow(row);
...
async function actionSendToPlayer(event, body) {
  ...
  const playerId = safeId(body.playerId || body.toPlayerId || body.toFriendId, 96);
  if (!playerId) {
    const error = new Error('player_required');
    error.httpStatus = 400;
    throw error;
  }
  const targetDeviceId = safeId(body.targetDeviceId, 120);
  const notification = {
  ...
  const results = [];
  for (const row of rows) results.push(await sendToSubscription(row, notification));
