...
const INTELLIGENCE_DATA_FILE = path.join(META_DIR, 'project-intelligence-data.txt');
const STALE_META_FILES = [
  path.join(META_DIR, 'project-adaptive.txt'),
  path.join(META_DIR, 'project-intelligence.txt'),
  path.join(META_DIR, 'project-analytics.txt'),
...
const SELF_INTELLIGENCE_DATA_REL = toUnix(path.relative(ROOT, INTELLIGENCE_DATA_FILE));
...
const INTELLIGENCE_DATA_FILES = Object.freeze([
  '.github/workflows/generate-context.yml',
  '.github/workflows/generate-track-profiles-index.yml',
  '.github/workflows/validate.yml',
  'albums.json',
  'service-worker.js',
  'generate-context.js',
  'data/custom_achievements.json',
  'data/listen-track-catalog.env.json',
  'data/listen-track-catalog.function-env.json',
  'data/taxonomy.json',
  ...
  'scripts/app.js',
  'scripts/app/gallery-recommendation-cards.js',
  'scripts/app/gallery.js',
  'scripts/app/recommendation-playback.js',
  'scripts/app/track-registry.js',
  'scripts/app/games/bridge-host.js',
  'scripts/app/profile/account-cloud-renderers.js',
  'scripts/app/profile/account-device-initialization.js',
  'scripts/app/profile/account-devices-view.js',
  'scripts/app/profile/account-timezone-view.js',
  'scripts/app/profile/achievements-view.js',
  'scripts/app/profile/actions.js',
  'scripts/app/profile/backup-info-modal.js',
  'scripts/app/profile/live-bindings.js',
  'scripts/app/profile/logs-formatters.js',
  'scripts/app/profile/logs-view.js',
  'scripts/app/profile/loyalty-card.js',
  'scripts/app/profile/model.js',
  'scripts/app/profile/profile-tab-renderers.js',
  'scripts/app/profile/recs-view.js',
  'scripts/app/profile/stats-view.js',
  'scripts/app/profile/template.js',
  'scripts/app/profile/view.js',
  'scripts/app/profile/yandex-actions.js',
  'scripts/app/profile/yandex-auth-view.js',
  'scripts/app/push/loyalty-reminders.js',
  'scripts/app/push/web-push.js',
  'scripts/app/pwa-install.js',
  'scripts/app/shards/reward-notifier.js',
  'scripts/app/shards/view.js',
  'scripts/app/shards/wallet-service.js',
  'scripts/ci/contracts/backup-analytics.mjs',
  'scripts/ci/generate-listen-catalog.mjs',
  'scripts/ci/generate-track-profiles-index.mjs',
  'scripts/ci/normalize-track-profile.mjs',
  'scripts/ci/test-backup-coordinator.mjs',
  'scripts/ci/test-backup-scheduler.mjs',
  'scripts/ci/test-recommendation-score.mjs',
  'scripts/ci/test-stats-v5.mjs',
  'scripts/ci/test-track-similarity.mjs',
  'scripts/ci/validate-contracts.mjs',
  'scripts/ci/validate-intel-profiles.mjs',
  'scripts/core/app-activity.js',
  'scripts/core/cloud-usage-meter.js',
  'scripts/core/config.js',
  'scripts/core/device-context.js',
  'scripts/core/device-identity.js',
  'scripts/core/favorites-manager.js',
  'scripts/core/social-session.js',
  'scripts/core/timezone-policy.js',
  'scripts/core/yandex-auth.js',
  'scripts/core/yandex-backup-v7.js',
  'scripts/offline/cache-db.js',
  'scripts/ui/progress-formatters.js',
  'scripts/ui/sleep-timer.js',
  'scripts/ui/statistics-modal.js',
  'scripts/ui/track-statistics-modal.js',
  'src/PlayerCore.js',
...
const FUNCTION_CLIENT_MARKERS = Object.freeze([
  'requestSocialAction',
  'getSocialSession',
  'YandexBackupV7',
  'YANDEX_BACKUP_V7_PROXY',
  'meteredJsonFetch',
  'ListeningReceipts',
  'PlaybackOwnership'
]);
...
const isExcluded = (rel, patterns) => {
  const u = toUnix(rel);
  if (!u || u === 'cloud-functions' || u.startsWith('cloud-functions/') || u === SELF_FULL_REL || u === SELF_MUSIC_GAMES_REL || u === SELF_INTELLIGENCE_DATA_REL || ALWAYS_EXCLUDE.has(u) || ALWAYS_EXCLUDE.has(path.basename(u))) {
    return true;
  }
...
const listTextFilesUnder = relativeDirectory => {
  ...
  while (stack.length) {
    ...
    for (const entry of fs.readdirSync(directory, { withFileTypes: true })) {
      ...
      if (entry.isDirectory()) stack.push(absolute);
        ...
        const relative = toUnix(path.relative(ROOT, absolute));
        if (isTextFile(relative)) files.push(relative);
      }
    }
  }
  return files.sort();
...
const findFunctionClientFiles = () => listTextFilesUnder('scripts')
  .filter(relative => !relative.startsWith('scripts/ci/') && !relative.startsWith('scripts/e2e/'))
  .filter(relative => {
    const source = readText(relative);
    return FUNCTION_CLIENT_MARKERS.some(marker => source.includes(marker));
  });

const intelligenceDataFiles = () => [...new Set([
  ...INTELLIGENCE_DATA_FILES,
  ...listTextFilesUnder('scripts/analytics'),
  ...listTextFilesUnder('scripts/intel'),
  ...findFunctionClientFiles(),
  ...CLOUD_FUNCTIONS.flatMap(name => listTextFilesUnder(`cloud-functions/${name}`))
])].filter(relative => fs.existsSync(path.join(ROOT, relative)) && isTextFile(relative)).sort();
...
const generateSelectedContext = ({ purpose, files }) => {
  const selected = [...new Set(files.map(toUnix))].sort();
  let out = headerBlock({ purpose });
  out += `ВАЖНЫЕ ФАЙЛЫ ЭТОГО КОНТЕКСТА:
${selected.map(relative => `- /${relative}`).join('\n')}

`;
  selected.forEach(relative => {
    out += fs.existsSync(path.join(ROOT, relative)) && isTextFile(relative)
      ? fileBlock(relative)
      : `//=================================================
// FILE: /${relative}
...
const generateMusicGames = () => generateSelectedContext({
  purpose: 'точечный контекст Game Center / iframe host / postMessage bridge / service worker / запуск /Games/',
  files: MUSIC_GAMES_FILES
});

const generateIntelligenceData = () => generateSelectedContext({
  purpose: 'рекомендации, TrackProfile, ListenerProfile, статистика, достижения, Осколки, Преданность, Backup V7, account data и все клиентские/серверные интеграции Yandex Cloud Functions',
  files: intelligenceDataFiles()
});
...
try {
  ...
  if (['intelligence', 'analytics', 'data', 'focused', 'both', 'all'].includes(MODE)) {
    fs.writeFileSync(INTELLIGENCE_DATA_FILE, generateIntelligenceData(), 'utf8');
    console.log(`✅ ${INTELLIGENCE_DATA_FILE}`);
  }
