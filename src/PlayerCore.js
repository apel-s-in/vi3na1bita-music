...
(function () {
  ...
  class PlayerCore {
    constructor() {
      ...
      //
      // ✅ Sources of truth:
      // - likedTrackUids:v2 (global uid set) => ⭐ стоит/не стоит
      // - favoritesRefsByUid:v2 (uid->ref)   => строка существует (active/inactive)
      //
      // ✅ Temporary back-compat (until PlaybackPolicy + e2e migrated):
      // - likedTrackUids:v1 (albumKey -> uid[]) is maintained in parallel.
      //
      // ❌ Legacy refs favoritesAlbumRefsByUid:v1 are NOT used by PlayerCore logic anymore.
      // They must exist only for FavoritesV2.ensureMigrated().
      this._fav = {
    ...
    // =========================
    // =========================
    // Favorites API (v2 source of truth + v1 liked back-compat)
    // =========================
    ...
    // Back-compat for PlaybackPolicy (still v1-based for now)
    ...
    /**
     * toggleFavorite(uid, { fromAlbum, albumKey })
     *
     * Rules (strict):
     * - fromAlbum=true:
     *   like  => ensure ref exists + liked=true
     *   unlike => remove ref полностью "без следа" + liked=false
     *
     * - fromAlbum=false (favorites view):
     *   like  => liked=true (ref stays/activates)
     *   unlike => liked=false BUT ref stays and becomes inactiveAt (inactive row)
     *
     * STOP разрешён только в одном сценарии:
     * playingAlbum == __favorites__ AND unlike сделан из favorites view по текущему uid:
     *   - если после unlike активных liked==0 => stop()
     *   - иначе => next()
     */
    toggleFavorite(uid, opts = {}) {
      ...
      // fromAlbum: true только если действие НЕ из favorites view
      ...
      // ✅ v2 toggle with strict source
      const source = fromAlbum ? 'album' : 'favorites';
      FavoritesV2.toggle(u, { source });

      // ✅ v1 liked map maintained temporarily (only if albumKey is known & regular)
      if (albumKey && !String(albumKey).startsWith('__')) {
        this._favSyncLikedV1ForAlbumUid(albumKey, u, nextLiked);
      } else {
        // best-effort: if we are in favorites view and track currently playing has sourceAlbum,
        // keep v1 in sync too (important for mini-player + e2e expectations).
        try {
      ...
      // Notify subscribers (UI/policy/offline indicators must not auto-play)
      try {
        this._favEmitter.emit({ albumKey: albumKey || '', uid: u, liked: nextLiked, fromAlbum: !!fromAlbum });
      } catch {}

      // ✅ STOP/next rule (ONLY for favorites playlist, ONLY when unlike from favorites view)
    ...
    /**
     * v2-only favorites state
     * Returns:
     * - { activeUids, inactiveUids }
     * + back-compat aliases { active, inactive } (arrays of {uid})
     */
    getFavoritesState() {
      FavoritesV2.ensureMigrated();
      ...
      // deterministic order: active first in UI usually, но тут только состояние
      activeUids.sort();
      inactiveUids.sort();

      // back-compat shape used in some places (offline/playback-cache bootstrap)
    ...
    removeInactivePermanently(uid) {
      ...
      // remove ref полностью (liked уже должен быть снят)
    ...
    restoreInactive(uid) {
      ...
      // v1 sync best-effort (if we can infer sourceAlbum from TrackRegistry/current track)
      try {
