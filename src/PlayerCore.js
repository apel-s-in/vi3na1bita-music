...
(function () {
  ...
  class PlayerCore {
    ...
    toggleFavorite(uid, opts = true) {
      ...
      // Определяем albumKey источника для likedTrackUids:v1:
      // - если играем __favorites__, то sourceAlbum лежит в track.sourceAlbum
      // - иначе используем playingAlbum (обычный альбом)
      const cur = this.getCurrentTrack();
      const sourceAlbum = (playingAlbum === W.SPECIAL_FAVORITES_KEY)
        ? safeStr(cur?.sourceAlbum)
        : safeStr(playingAlbum);

      // Если это special/нет sourceAlbum — всё равно позволяем "toggle" как UI-state, но без записи map
      const map = this._favReadLikedMap();
      ...
      const albumKey = sourceAlbum || findAlbumContainingUid() || null;
      const prevLiked = albumKey ? this.getLikedUidsForAlbum(albumKey).includes(u) : this.isFavorite(u);
      const nextLiked = !prevLiked;
