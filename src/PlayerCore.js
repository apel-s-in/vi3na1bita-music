...
(function () {
  ...
  class PlayerCore {
    ...
    async load(index, opts = {}) {
      ...
      const pos = Number(opts.resumePosition) || 0;

      const retry = {
        n: Number(opts._retryN) || 0,
        max: 2
      };

      this._unload(true); // Silent Hot Swap

      const isStreaming = !!(res?.source === 'stream');

      this.sound = new Howl({
        src: [url],
        // R0 hardening: stream via HTML5 Audio (iOS/Safari stability), local blobs keep WebAudio.
        html5: isStreaming,
        ...
        onend: () => tok === this._tok && (
          this._emit('onEnd'),
          this._updMedia(),
          W.dispatchEvent(new CustomEvent('player:ended')),
        ...
        onloaderror: () => {
          ...
          const netAllowed = W.NetPolicy?.isNetworkAllowed?.() ?? navigator.onLine;
          if (netAllowed && retry.n < retry.max) {
            const delay = 250 + retry.n * 500;
            setTimeout(() => {
              if (tok !== this._tok) return;
              this.load(index, { ...opts, autoPlay: true, _retryN: retry.n + 1, dir });
            }, delay);
            return;
          }

          // No-stop invariant: auto-skip forward
          this._skips.count = Math.min(this._skips.max, (this._skips.count || 0) + 1);
          setTimeout(() => {
            if (tok !== this._tok) return;
            this.load((index + dir + this.playlist.length) % this.playlist.length, {
              ...opts,
              autoPlay: true,
              isAutoSkip: true,
              dir
        ...
        onplayerror: () => {
          if (tok !== this._tok) return;
          this._emit('onPlaybackError', { reason: 'playerror' });
    ...
    _unload(silent) {
      ...
      // IMPORTANT: do not broadcast "stop" on silent hot swap (track change / retry).
      if (!silent) {
